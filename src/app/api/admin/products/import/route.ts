...
/** Admin product import: 20 imports per 15 minutes per user (bulk sessions can legitimately import several files). */
const IMPORT_LIMIT = { max: 20, windowMs: 15 * 60 * 1000 };

/**
 * POST /api/admin/products/import — bulk product CSV import (Session 51).
 * Admin-only. Body: { csv: string } (raw CSV text; the server is the
 * authoritative parser). Returns a per-row ImportReport. Create-only —
 * duplicate slugs are skipped, never overwritten. New endpoint: the existing
 * product POST/PUT/DELETE routes are untouched.
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await executeProductImport(csv);
