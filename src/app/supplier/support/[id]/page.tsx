...
import {
  useSupplierConversation,
  useSupplierSendConversationMessage,
} from "@/hooks/use-supplier-conversations";
...
export default function SupplierSupportDetailPage({
  params,
}: {
  ...
  const { data: conversation, isLoading, isError, refetch } = useSupplierConversation(id);
  const send = useSupplierSendConversationMessage();
      ...
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <Button variant="outline" onClick={() => router.push("/supplier/support")}>
              بازگشت به گفتگوها
            </Button>
          </CardContent>
        </Card>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex items-center gap-2 text-sm text-muted-foreground">
        <Link href="/supplier/support" className="transition-colors hover:text-foreground">
          ارتباط با مشتریان
        </Link>
      ...
      {/* Thread + composer (supplier is reply-only — no status controls) */}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <ConversationThread
            ...
            viewerRole="supplier"
            peerLabels={{ customer: getCustomerName(conversation.customer) || "مشتری" }}
            disabled={closed}
            disabledHint="این گفتگو بسته شده است"
            placeholder="پاسخ مشتری را بنویسید... (Ctrl+Enter برای ارسال)"
            onSend={handleSend}
