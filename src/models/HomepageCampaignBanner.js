...
/**
 * HomepageCampaignBanner — Session 53 Homepage CMS, Tier 2 content model for
 * the campaign banner section. Bound to a section instance by `sectionSlug`;
 * "activate one banner" = isActive + lowest sortOrder wins.
 *
 ...
 */
const HomepageCampaignBannerSchema = new mongoose.Schema(
  {
    sectionSlug: {
      type: String,
      required: true,
...
HomepageCampaignBannerSchema.index({ sectionSlug: 1, sortOrder: 1 });
HomepageCampaignBannerSchema.index({
  sectionSlug: 1,
  status: 1,
  isActive: 1,
  deletedAt: 1,
...
export default
  mongoose.models.HomepageCampaignBanner ||
  mongoose.model("HomepageCampaignBanner", HomepageCampaignBannerSchema);
