...
> **Last Updated:** September 2026 (Session 89 — Production Readiness: Next 16 proxy, health endpoint, deployment & CI build gate; Session 88 also re-verified against the new database)
...
- **Current Milestone:** Session 88 — Profitability Report Completion (سودآوری) ✅ (code complete; API/E2E/regression gates pending the database being reachable again)
- **Next Milestone:** (1) re-run the three blocked Session 88 gates (`scripts/verify-reports.js` over HTTP, Playwright, full regression) as soon as MongoDB is reachable; (2) then **historical category profitability** — the one remaining profitability gap, which needs a `category` snapshot on the embedded `OrderItemSchema` + a backfill decision (migration-class, deliberately NOT part of Session 88); (3) the remaining Production Readiness tranches (deployment, monitoring, Core Web Vitals/Lighthouse CI) or the next Growth feature (customer order SMS notifications, customer segments → targeted coupons) — see ROADMAP.md / NEXT_SESSION.md
...
| **SmsLog** | recipient (canonical 09…), user/order/template (optional refs), **templateName snapshot** (survives template deletion), messageType (same enum as SmsTemplate.type), provider (mock/smsir/none), providerMessageId, status (sent/failed — written AFTER the provider returns, no dangling queued state), error + errorMessage (controlled codes), message (rendered audit copy), sentAt, createdBy actor, timestamps | Session 90 — business-SMS audit trail (Phase 1a); query indexes (createdAt · status+createdAt · messageType+createdAt · recipient+createdAt · template); **intentionally NO TTL** (audit rows are never auto-deleted) and **intentionally NO dedupe key** (manual sends are explicit admin actions; order-event dedupe belongs to the deferred automation phase) |
