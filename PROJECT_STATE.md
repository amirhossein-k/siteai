...
> **Last Updated:** September 2026 (Session 91 — Business SMS Order-Event Automation, committed `4041927`, CI-green, deployed & production-smoked)
> **Docs gap recorded:** Sessions 85–87 were code-complete but left undocumented; Session 88 records them retroactively in CHANGELOG.md (reconstructed from their commit messages) — see the ⚠️ entries there.
...
- **Current Milestone:** Session 91 — Business SMS Order-Event Automation ✅ (committed `4041927`; CI run `35613757314` all four jobs PASS; Deploy run `35614725034` PASS; production smoke 200/`db:up`/PM2 online — production runs exactly `4041927`)
- **Next Milestone:** (1) **Dependabot security pass** — 22 findings on the default branch (2 critical · 14 high · 6 moderate), deliberately deferred to a dedicated session; (2) **historical category profitability** — the one remaining profitability gap, which needs a `category` snapshot on the embedded `OrderItemSchema` + a backfill decision (migration-class, deliberately deferred); (3) real SMS.ir business-pattern activation (register patterns in the sms.ir dashboard + real `providerTemplateId` values) and campaign/scheduling work; (4) local dev-DB hygiene — stale `e2e_1789952655421` Playwright fixture rows remain in the shared `marlooai` DB (caused a deterministic local accounting-excel FIFO-layer failure; CI's fresh per-run Mongo is unaffected) — see ROADMAP.md / NEXT_SESSION.md
- **Deployment:** **Deployable & verified (Session 89) — not yet rolled out live.** The production path is `npm ci && npm run build && npm start` (Chabokan's managed **NextJs** service runs those same `package.json` scripts and keeps env/Node/port in its dashboard; `chabok deploy`/git ships it — **no platform manifest is needed**, so no `vercel.json`/Dockerfile exists). `npm run build` exits 0 and is **DB-independent**; `"engines": { "node": ">=20.9.0" }` guards the runtime; `GET /api/health` (200/`db:up`, 503/`db:down`, uncached, non-mutating, 4s-bounded) is the health probe; `ci.yml` gained a `build` merge gate; `scripts/verify-deployment.js` proves the artifact + production boot (**7/7**); `README.md` is the runbook. **The live rollout is an operator action.**
- **Production readiness ✅ (Session 89 — `src/proxy.js` + `src/lib/health.ts` + `/api/health` + `scripts/verify-{health,deployment}.js` + CI `build` gate + README runbook):** the audit's #1 gap ("no verified production build/deploy/monitoring") closed for everything not needing a third-party account. **Proxy migration** — Next 16 renamed the `middleware` file convention, so `src/middleware.js` became `src/proxy.js` with its export renamed to `proxy`; the deprecation warning on every build is gone, and matcher/both redirect targets/RBAC decisions are byte-identical (git rename `R081`, route-guard E2E **25/25**). **Production build** — `npm run build` exit 0 (123 static pages) and **DB-independent** (proved with an unreachable `MONGODB_URI`), which is what makes the new hermetic CI job possible. **Deployment model established from the platform's documentation, not assumed** — the target is Chabokan's managed **NextJs** cloud-hosting service, which runs the project's own `build` + `start` scripts and holds env/Node/port in its dashboard, so **no `vercel.json`/Dockerfile was invented** (that host reads neither); the only production fact added is `"engines": { "node": ">=20.9.0" }` (Next 16's floor vs Chabokan's 14/16/18 options). **`GET /api/health`** — public, uncached, non-mutating (Mongo `ping` only), **4 s-bounded** (the driver default would be 30 s), **200**/`db:up` vs **503**/`db:down` (never a false 200), closed secret-free payload; verified live both ways (negative: **503 @ 4077 ms**). **CI** — `.github/workflows/ci.yml` gained a fourth merge-gating job `build` (`npm ci` + `npm run build`) with CI-only placeholder env; static/unit/e2e untouched; no Sentry. **Verification** — `scripts/verify-health.js` **9/9** (wired into `run-regression.js` → **50 suites**) and `scripts/verify-deployment.js` **7/7** (artifact, real production boot on its own port, health, production-vs-dev static-cache discriminator, no-secret server-log scan, no legacy Mongo target; intentionally outside the dev-server-driven regression). **Docs + credential hygiene** — `README.md` is now the runbook; `.env.example` was correctly gitignored (nothing in git history) but on disk held the **real** environment (11/12 values byte-identical to `.env.local`) and is now a placeholders-only template, with `!.env.example` added to `.gitignore` so the sanitized file is committable while `.env.local` stays ignored. Gates: tsc 0 · check exit 0 (4 pre-existing warnings) · Vitest **631/631** · route-guard E2E **25/25** · verify-health 9/9 · verify-deployment 7/7 · full **50-suite** regression. **Excluded by decision:** Sentry/monitoring (future session) and the live rollout (operator action).
- **Password recovery ✅ (Session 84 — «بازیابی رمز عبور» on the existing OTP infrastructure):** purpose-scoped `password_reset` OTP (enum + additive `resetTokenHash`/`resetTokenExpiresAt` on `OtpCode`; login/register contracts untouched; shared per-phone OTP/SMS budget); **closed anti-enumeration cooldown oracle** (unknown phones get the same 200 AND a dummy row → identical 60s resend cooldown, no SMS, no existence leak); verify mints a **256-bit single-use `resetToken`** (hashed at rest, 2-min TTL, React-memory only — never a `loginToken`, never `signIn()`: a recovery cannot create a session); `POST /api/auth/password-reset/complete` — token-gated bcrypt set (same 6–100 policy), **`tokenVersion` bump + cache eviction** (all prior sessions revoked), and **outstanding-OTP invalidation** (a pending pre-reset login token can never authenticate); passwordless accounts may set their FIRST password; uniform 400 for unknown token/account; 5/phone/15min. Four-step `/forgot-password` UI in the Session 83 language + «رمز عبور را فراموش کردهاید؟» login entry. **Real bug fixed:** the shared `rate-limiter.ts` find-then-create race (concurrent first-requests → E11000 → 500; now E11000-guarded). `verify-password-reset.js` **22/22 real-API** → **49-suite regression**; unit 10 (**580/580** Vitest); Journey 18 E2E (desktop full loop + wrong-OTP; mobile **anonymous** smoke that never registers — register-budget discipline); auth E2E subset **36/36**; Playwright **122/123** (single failure = the pre-existing external-Parspack rich-description flake, reproduced at clean HEAD) · full regression **48/49** (single failure = the pre-existing Zarinpal-sandbox flake in `verify-payment-retry`, passes 16/16 standalone); git diff --check clean. NOT committed — awaiting approval.
- **Testing:** Inventory concurrency verified (10/10 tests) ✅, API pagination verified (24/24 tests) ✅, attributes & variants verified (16/16 tests) ✅, **attributes & variants E2E verified against the real HTTP API (32/32 tests) ✅**, rate limiting tested ✅, 401/403 auth tested ✅, **upload repro 15/15 ✅, upload formats 9/9 ✅**, **payment retry & abandoned cleanup 14/14 ✅ (real API + Zarinpal sandbox)**, **variant polish 14/14 ✅ (real API)**, **admin refund flow 12/12 ✅ (real API)**, **supplier payout approval 17/17 ✅ (real API)**, **customer reviews & ratings 20/20 ✅ (real API)**, **customer wishlist 14/14 ✅ (real API)**, **customer notifications 18/18 ✅ (real API)**, **supplier review replies 21/21 ✅ (real API)**, **wishlist → cart bulk move 18/18 ✅ (real API + real cart store)**, **discounts & coupons 27/27 ✅ (real API)**, **real-time notifications SSE 11/11 ✅ (real API)**, **admin analytics & reporting 16/16 ✅ (real API)**, **supplier storefront pages 20/20 ✅ (real API)**, **variant-level wishlist 19/19 ✅ (real API)**, **coupon marketing surface 12/12 ✅ (real API)**, **db-outage resilience 9/9 ✅ (hermetic, in-process — `verify-db-reconnect.js`)**, **supplier telegram alerts 16/16 ✅ (real API — `verify-telegram-alerts.js`)**, **customer self-service order cancellation 17/17 ✅ (real API — `verify-order-cancel.js`, incl. strict cancel-vs-payment-verify race)**, **storefront faceted filtering (brand + tag) 22/22 ✅ (real API — `verify-facets.js`, incl. combination filters, 404/empty semantics, leak scan)**, **attribute facets 24/24 ✅ (real API — `verify-attribute-facets.js`, incl. nested `attributes[<slug>]` filters, aggregation counts, sticky self-exclusion)**, **search quality upgrade 17/17 ✅ (real API — `verify-search.js`, incl. expanded coverage name/description/attribute-value/brand/tag/category, weighted relevance ranking exact>prefix>substring>brand/tag/category>attribute>description, explicit-sort override, pagination stability, Persian partial substring, regex special-char escaping, backward-compat keyset + no `score` leak)**, **search suggestions 10/10 ✅ (real API — `verify-search-suggest.js`, incl. public endpoint, prefix match, inactive excluded, brand names, product-before-brand order, dedup, max 10 cap, special-char resilience)**, **homepage UX redesign ✅ (browser QA desktop + mobile: all sections render, header search suggestions live, category tiles pre-filter the catalog, mobile scroll-snap + expanding search row, countdown chip, zero console errors — no new API suite needed)**, **bulk product CSV import/export 22/22 ✅ (real API — `verify-product-import-export.js`, incl. admin + supplier import/export, create-only skip-duplicates in-file/in-DB, per-row atomicity + failure isolation, name-based ref resolution, Persian-digit normalization, supplier auto-ownership + ownership-scoped export, export→import round-trip, formula-injection escaping, sanitization, byte/row caps, rate limit)**, **mobile dashboard navigation fix ✅ (Session 52 — reusable `MobileDrawer` on admin + supplier dashboards; the `Menu` button previously toggled an un-consumed store flag; browser QA at 390px confirmed open/close/navigate; desktop sidebar byte-identical; no API/schema change, no new suite needed)**, **homepage CMS 14/14 ✅ (real API — `verify-homepage-cms.js`, incl. admin authz 401/403, idempotent sections bootstrap + static-config seed, public composition order + strict-projection leak scan, unknown-registry-component fail-safe, shared-renderer isolation, slug/component immutability, enabled/draft/soft-delete visibility rules, publishedAt stamping, per-type CRUD + validation, malformed ObjectId → 400)**, **coupon eligibility 18/18 ✅ (real API — `verify-coupon-eligibility.js`, Session 55, incl. assigned_users single + multi create, user_groups fail-closed until groups exist, non-assigned checkout → 400 with NO order/claim, assigned checkout exact discount, PUT audience reassign, public-list leak scan for eligibility fields, populated admin GET)**, **best-sellers rail 14/14 ✅ (real API — `verify-best-sellers.js`, Session 56, incl. sort=best_selling ranking + newest tie-break, list/detail leak scans (soldCount never exposed), refund + admin-cancel-of-paid reversals by exact quantities, legacy 0 floor never negative, double-refund 400 no double decrement, pending orders never counted, cash checkout never increments, CMS best-sellers block seeded + present in public composition; OPTIONAL `scripts/backfill-sold-count.js` backfill)**, **order management v2 17/17 ✅ (real API — `verify-order-management.js`, Session 57, incl. atomic claim-based admin transitions, claim-guard vs payment-verify race (never `failed` on a paid order), shipping subdocument on shipped/delivered with optional trackingCode + sanitization, actor-labeled history, refund payment-domain separation, paid-cancel soldCount reversal, admin list sort=newest/oldest, customer-cancel invariants, authz)**, **unit test foundation 158/158 → 183/183 ✅ (Vitest — Session 58 foundation + Session 62 additions: `npm test`; hermetic suites over the src/lib helpers — sanitize, utils, pagination, product-variants, product-csv, coupons, inventory, product-sales, payment-cleanup, **otp (Session 62)**, **sms (Session 62)** — Mongoose models mocked (`vi.mock` + `vi.hoisted`), no DB/network; target libs 92–100% line coverage, report-only)**, **playwright E2E 21/21 ✅ (Session 59 — `npx playwright test` (chromium 16 + chromium-mobile 5, exit 0): 10 journeys — customer login (real UI form), product search (suggestions + catalog), product detail (simple + variants), cart (incl. mobile smoke), checkout, coupon, payment (hermetic `ZARINPAL_MOCK` seam), order tracking, admin order workflow (lifecycle + shipping metadata), supplier workflow; real API logins + storageState; per-run PREFIX seeding + teardown cleanup; the only `src/` change is the dev-gated `ZARINPAL_MOCK` seam in `src/lib/zarinpal.ts`; regression suites unaffected — they run against the real sandbox with the env unset)**, **performance & accessibility ✅ (Session 61 — storefront **next/image** migration: 12 image components / 17 `<Image>` instances with `fill` aspect containers + explicit `sizes` + critical-only `eager` loading; `next.config.ts` `images.remotePatterns` additive allowlist from env (Liara endpoint w/ known fallback host, app URL, localhost); axe-core accessibility fixes (search-suggestions `aria-selected`, image-lightbox labelled backdrop/zoom buttons, storefront-header icon-link `aria-label`s); `@axe-core/playwright` + `tests/e2e/accessibility.spec.ts` Journey 11 — axe wcag2a/2aa/21a/21aa over homepage/catalog/product-detail/supplier-detail/login/register asserting **zero serious/critical violations** (inside `npm run e2e` → inside the CI e2e gate); `PERFORMANCE.md` documents the pipeline + the out-of-scope admin/supplier native `<img>` debt; zero business-logic/model/API changes — Playwright 27/27, Vitest 158/158, no dev-server restart)**, **CI/CD pipeline ✅ (Session 60 — `.github/workflows/ci.yml`: push/PR merge gates — static (`tsc` + scoped eslint `src/lib tests/unit tests/e2e`, 0 errors), unit (Vitest 183/183), e2e (Playwright 30/30 against a fresh `mongo:7` service container with `ZARINPAL_MOCK=1` + `SMS_MOCK=1`, **zero CI secrets** — `global-setup` auto-seeds the admin + per-run users); optional non-gating `.github/workflows/regression.yml` nightly 35-suite real-sandbox regression, secrets-driven + self-skipping; `npm run check` = the static job locally; verified-dead `src/hooks/useOrders (1).js` deleted; **zero `src/` business-logic changes**), **SMS/OTP authentication ✅ (Session 62 — additive OTP login/registration: `OtpCode` model + optional `User.passwordHash` + `tokenVersion`, adapter-first `src/lib/sms.ts` (SMS_MOCK dev/CI seam · sms.ir production opt-in · controlled 503 when unconfigured), `/api/auth/otp/{request,verify,dev-last}`, replay-proof `loginToken` branch in the CredentialsProvider, login/register OTP tabs (password default); rate limits + 60s cooldown + 5-attempt lock + login anti-enumeration; `verify-otp.js` 11/11; Journey 12 E2E; 25 new unit tests; password auth byte-for-byte unchanged)**, **logout / sign-out ✅ (Session 63 — storefront account menu `src/components/storefront/account-menu.tsx` replaces the header's profile link when signed in (پروفایل/سفارشات/علاقه‌مندی‌ها/اعلان‌ها + «خروج»; `aria-haspopup`/`aria-expanded`/`role=menu`; Escape + outside-click + route-change close; desktop + mobile tap) + «خروج از حساب» button on the profile page; admin/supplier sidebar logout unchanged (desktop + mobile drawer); logout rides the built-in NextAuth `POST /api/auth/signout` → JWT cookie cleared, redirect `/` for all roles; **zero auth/API/model/middleware/RBAC/OTP/password changes**, no new packages, no restart; Journey 13 E2E (`logout.spec.ts`, 6 tests × both projects incl. admin/supplier post-logout `/admin`+`/supplier` → `/login` guards) + `verify-logout.js` 6/6 (35-suite regression); Playwright 42/42 → **44/44** after Session 63.1 — wishlist header entries (desktop nav link + heart + account-menu item) gated to `role === "customer"` (page/API are customer-only — authenticated admins previously hit the sign-in prompt; customers proven working) and the «کدهای تخفیف» nav entry session-gated (hidden for anonymous); Journey 13 grew to 7 tests; zero auth/API/model/middleware/RBAC/OTP/password changes)**, **session security ✅ (Session 64 — `tokenVersion` enforcement, the declared Session 62 deferred item: pure `token-version.ts` checker — per-user 60s cache, asymmetric semantics, deleted-user revoked, error fail-open — wired into `auth-utils.getServerToken` so every protected API 401s revoked JWTs; `POST /api/auth/change-password` (bcrypt current-password check; passwordless OTP users set their first password; bump → all sessions revoked), `POST /api/auth/logout-all` (bump → every device dead), `POST /api/admin/users/[id]/revoke-session` (admin-only, malformed → 400, unknown → 404); `GET /api/profile` additive `hasPassword`; profile «امنیت حساب» card (change/set password + logout-all, proper `htmlFor`/`id`); bumps evict the cache for immediate in-process revocation; **zero auth-flow/middleware/RBAC/model/business changes, zero new packages**; Journey 14 E2E (4 tests) + `verify-session-security.js` 11/11 (36-suite regression); Vitest 199/199, Playwright 48/48)**, **customer communication ✅ (Session 68 — the long-listed ❌ «Customer support (ticket system)» feature: order-linked `CustomerConversation` model (customer/order/supplierOrder/supplier, optional product, category, subject, status open|pending|resolved|closed, unread flags, messages[] with sender role, resolvedAt/closedAt; **unique partial index `{supplierOrder} where active`** — one thread per customer × supplier-order, cross-supplier leakage structurally impossible); **11 additive endpoints** (customer `/api/conversations*` — own paid orders only, supplier from the SupplierOrder; admin `/api/admin/conversations*` — full access, filters + search; supplier `/api/supplier/conversations*` — own threads only, reply-only) all `requireRoleOrError` + same-404 not-found/not-owned + malformed→400 + validation-before-rate-limit (create 5/user/15min, message 15/actor/15min); notifications via the reused `notifyOrderEvent` facade — `support` category + `support_message` type, per-message dedupe, templated no-content payloads, SSE bell + inbox tab; UI customer `/support` + admin `/admin/support` + supplier `/supplier/support` + shared `src/components/support/`; real-time = SSE + 15s polling, no WebSockets; `verify-customer-support.js` 23/23 → **39-suite regression**; Journey 17 E2E; Vitest 254/254, Playwright 52/52)**, **supplier onboarding ✅ (Session 66 — dedicated `/admin/suppliers` page (create via shared CreateUserModal / promote customer via searchable modal / deactivate+reactivate / status + payouts link; sidebar «فروشندگان»); **zero duplicate supplier API** — create/promote reuse the existing `POST`/`PATCH /api/admin/users` (auto Supplier doc provisioning preserved); additive `GET /api/admin/suppliers?all=true` management shape (default dropdown unchanged); **deactivation enforcement** — `toggle-active` now flips linked `Supplier.isActive` (public storefront hides it) + bumps `tokenVersion` (sessions revoked); **role-change session revocation** — any `change-role` bumps `tokenVersion`; shared `create-user-modal.tsx`; `AdminSupplier` type + `use-admin-suppliers.ts`; **no public registration / application queue (out of scope)**; `verify-suppliers-onboarding.js` 14/14 (401/403, create+auto-provision, promote+session-revoke, deactivate/reactivate, public hiding, dropdown-shape guard) → **37-suite regression**; Journey 15 E2E (2 tests); Vitest 211/211, Playwright 50/50, regression 37/37)**, **public supplier application + admin approval queue ✅ (Session 67 — closes Session 66's explicitly out-of-scope gap: an additive public path where the applicant STAYS `role: customer` until admin approval — self-role-assignment impossible by construction. New `SupplierApplication` model (user ref / businessName 2–80 / description ≤500 / contactPhone / status pending|approved|rejected / adminNote / decidedBy+decidedAt; **unique partial index** `{user,status} where status=pending` — one open application per user, E11000 → 409; rejected may re-apply; zero changes to User/Supplier). Public `POST /api/supplier-applications` (customer-only, rate-limited 2/user + 5/IP per 15min, pending-dedup 409, notifies all admins, **never touches role/Supplier**) + `GET /api/supplier-applications/me`; admin queue `GET + PATCH /api/admin/supplier-applications` (admin-only, 30/actor/15min) — **atomic approve/reject**: approve seeds the Supplier doc **from the application** (businessName/description), flips role, **bumps tokenVersion + evicts cache** (old customer sessions revoked instantly), sets decision metadata, notifies the applicant; reject leaves role untouched. Single provisioning implementation extracted to `src/lib/supplier-provision.ts` (Session 66 change-role refactored onto it, defaults preserved). Notification type enum gains `supplier_application`/`supplier_approved`/`supplier_rejected`; **model change ⇒ restart** (done). Public UI: `/become-supplier` page (anonymous → sign-in prompt; customer → form + status view; approved → note shown) + footer link + account-menu item + `/suppliers` CTA; admin UI: «درخواستهای فروشندگی» tab on `/admin/suppliers` (pending cards with approve/reject + note, decided history renders even when queue empty — early-return bug caught by Journey 16 and fixed). `verify-supplier-applications.js` 16/16 real-API (401/403, validation, dedup 409, role-untouched until approve, seeded-from-application Supplier, old-session 401, reject keeps role, re-apply, notifications) → **38-suite regression**; Journey 16 E2E (dedicated per-run applicant so the shared seeded customer's tokenVersion is never bumped); Vitest 226/226 (211 + 15 unit), Playwright 51/51 (chromium 39 + mobile 12), regression 38/38)**
- **Business SMS order-event automation ✅ (Session 91 — `src/lib/sms-order-events.ts` + additive `Order.smsEvents` markers + SmsLog state machine):** the five real lifecycle events (ORDER_CREATED · PAYMENT_SUCCESS · ORDER_SHIPPED · ORDER_DELIVERED · REFUND_COMPLETED) fire business SMS through the Session 90 service with durable, idempotent delivery semantics — markers pushed in the SAME atomic write as each transition (pre-generated `_id` in `Order.create`; `$push` inside the pending→paid / status-claim / paid→refunded claims), deterministic dedupe keys `order:<orderId>:<suffix>`, unique partial index on `SmsLog.dedupeKey` as the insert gate, SmsLog `pending→sending→sent|failed|unknown` with single-document CAS send/finalize (no multi-document transactions, no worker/cron); pending marker + `sent` SmsLog ⇒ zero provider calls + marker repaired; no automatic stale-`sending` reclaim; `unknown` terminal for automation; disabled provider / missing-inactive template → marker stays pending, no provider call; `fireOrderSmsEvent` never throws so SMS failures cannot break order/payment/shipping/refund; phones/names server-derived; templates by application-level NAME with `providerTemplateId` only from the stored document; `verify-sms-order-events.js` **9/9** · unit **18/18** · Vitest **687/687** · **CI `35613757314` 4/4 PASS · Deploy `35614725034` PASS · production smoke PASS (production = `4041927`)**
- **Admin business-SMS management ✅ (Session 90 Phase 1a+1b+1c — `SmsTemplate` + `SmsLog` models (no TTL / no dedupe by design), independent `src/lib/sms-business.ts` service (mock-first dev/CI seam · smsir `/v1/send/bulk` **only** behind the explicit `SMS_BUSINESS_ENABLED=1` kill-switch — production business SMS disabled by default · controlled never-throw failures · pure `renderTemplate` with fail-closed variables), **3 admin-only API surfaces** (`/api/admin/sms/templates` CRUD with server-derived variables · `/api/admin/sms/send` manual template/free-form send with strict phone normalization + inactive-template refusal + missing/extra-variable validation · `/api/admin/sms/logs` paginated filters) with independent rate limits (`sms-send:` 10/admin/15min, `sms-template-write:` 30/admin/15min — OTP limiter keys untouched), `/admin/sms` UI (قالب‌ها / گزارش ارسال / ارسال دستی tabs) + sidebar «پیامک‌ها»; **OTP isolation contract honored** — `sms.ts`, `otp.ts`, all OTP routes, `auth.js`, password-reset, `OtpCode` and every OTP test byte-for-byte untouched; `verify-sms.js` **17/17** (×2) → **51-suite regression**; Vitest 669/669; admin-sms E2E 3/3; OTP suites green after the change (13/13 · 22/22 · E2E 9/9)**

**Regression runner:** `scripts/run-regression.js` now runs **51 suites** sequentially (Session 90 added `verify-sms` — admin business-SMS management — after `verify-accounting-excel`; Session 89 added `verify-health` — the `/api/health` readiness probe — as the **second** suite, right after the hermetic `verify-db-reconnect`) (Session 62: `verify-otp` after `verify-homepage-cms`; Session 63: `verify-logout` after `verify-otp`; Session 64: `verify-session-security` after `verify-logout`, before `verify-telegram-alerts`; Session 66: `verify-suppliers-onboarding` after `verify-suppliers`; Session 67: `verify-supplier-applications` after `verify-suppliers-onboarding`, before `verify-variant-wishlist`; Session 68: `verify-customer-support` after `verify-supplier-applications`; Session 80: `verify-admin-notifications` after `verify-customer-support`; Session 81: `verify-reports` after `verify-admin-notifications`; Session 82 Phase A: `verify-accounting` after `verify-reports`; Session 82 Phase B: `verify-purchases` after `verify-accounting`; Session 82 Phase C: `verify-fifo` after `verify-purchases`; Session 82 Phase D: `verify-inventory` after `verify-fifo`; Session 82 Phase E: `verify-expenses` after `verify-inventory`; Session 84: `verify-password-reset` after `verify-supplier-applications`, before `verify-variant-wishlist`) — `verify-db-reconnect` (hermetic, no server/DB needed) is the **first suite**, so it runs even when the dev server is down (HTTP suites then SKIP, exit 1); `verify-facets` after verify-variant-polish + `verify-attribute-facets` after verify-facets + `verify-search` after verify-attribute-facets + `verify-search-suggest` after verify-search + `verify-product-import-export` after the variant group (before verify-coupons-marketing); `verify-coupons-marketing` near-last + `verify-coupon-eligibility` (Session 55) + `verify-homepage-cms` (Session 53) + `verify-otp` (Session 62) + `verify-telegram-alerts` last; **Session 52/62/68/82 hermeticity fix:** the runner clears the shared login + OTP + conversation + accounting rate-limiter keys (`_id: /^rl:(login|login_ip|otp_request|otp_request_ip|otp_verify|conversation-create|conversation-msg|accounting-init|accounting-config|password_reset_complete):/` on `ratelimits`) before EACH suite — the limits used to accumulate across suites and 401/429 later ones mid-run (production limiter untouched; `verify-purchases` self-cleans its own `purchase-write` keys; `verify-password-reset` self-sweeps `register` fixture keys + crashed-run residue)
- **Accounting-ready reports ✅ (Session 81 — `src/lib/reports.ts` + `src/lib/report-utils.ts` + `src/lib/reports-excel.ts` + `/admin/reports` UI):** 9 admin-only aggregation services (dashboard/sales/orders/payments/refunds/coupons/customer-sales/inventory/P&L); sales/COGS/gross profit from **immutable order snapshots** (line `price`/`originalPrice`/`discountAmount`, `supplierPrice` = historical COGS — current product prices never used for sales/profit; **inventory value is the documented exception**: current stock × current supplierPrice, a current-cost approximation, not FIFO/weighted-average purchase costing); order-level coupon discounts allocated proportionally by line so Σ line net reconciles to Σ `totalAmount`; professional multi-sheet `.xlsx` export via new `exceljs` dep (styled headers, freeze panes, auto-filter, number/date/percent formats, totals; `REPORT_EXPORT_LIMIT` 10/actor/15min); honest boundaries — taxes/shipping-cost/expenses/net-profit omitted or labeled «در دسترس نیست» (never fake zeros); zero model/schema changes, no migration; `verify-reports.js` 21/21 real-API → **42-suite regression**; unit 25 (497/497 Vitest); Journey E2E 4/4; Playwright 100/101 (single failure = pre-existing rich-description external-image flake, reproduced at clean HEAD); regression **42/42**
- **Procurement ✅ (Session 82 Phase B — `src/models/PurchaseOrder.js` + `src/lib/purchase-{math,prepare,view}.ts` + `/admin/purchases` UI + `verify-purchases.js`):** PurchaseOrder CRUD (draft→ordered→partially_received→received + cancelled; `P-YYYYMMDD-NNN` numbers; supplier ref; paymentStatus/amountPaid financially separate from receiving); **only receiving creates stock + FIFO cost layers + `receipt` InventoryMovements** (unit cost from the PurchaseItem, never current supplierPrice); partial + **idempotent** receiving (per-receipt client key, over-receive 409, concurrent dup-key → retry); consignment purchase → 400 (sourcing invariant preserved); purchases report added to Session 81 subsystem + Excel sheet; `PURCHASE_WRITE_LIMIT` 60/actor/15min; `verify-purchases.js` 16/16 → **45-suite regression**; unit 15 (528/528 Vitest); Journey E2E 4/4; Playwright 93/93 + mobile 12/12; regression **44/44** (server must run `SMS_MOCK=1` only — `ZARINPAL_MOCK` breaks the real-sandbox suites)
- **FIFO checkout integration ✅ (Session 82 Phase C — `src/lib/inventory.ts` + `src/app/api/checkout/route.ts` + `src/models/Order.js` + `src/lib/reports.ts` + `verify-fifo.js`):** purchased-sourcing sales consume FIFO cost layers in the SAME atomic update as the stock `$inc` (stockVersion + retry — concurrent checkouts never consume the same layer; verified 5+5 on [5×100k,5×200k]); **`OrderItem.fifoUnitCost`** additive snapshot (exact weighted FIFO cost; `supplierPrice` untouched; consignment/pre-cutover = null); **supplier payout safety** — purchased units create NO SupplierOrder (consignment keeps the existing payout flow); rollback + payment-failure/cancel/refund restoration return units to their layers at the snapshot cost (`return_restock`/`cancellation_restock` movements, idempotent); append-only `sale` movements; **insufficient layers → fail-safe 409**; **Reports V2** — COGS = `fifoUnitCost` else `supplierPrice` snapshot, inventory VALUE = Σ remaining × unitCost across layers for post-cutover purchased products (unlayered keeps the current-cost approximation); fixed pre-existing Session 81 bug (sales summary `unitsSold` was permanently 0); `verify-fifo.js` 15/15 → **45-suite regression**; unit 9 (537/537 Vitest); Journey 20 E2E 4/4; Playwright 109/109 + mobile 12/12; regression **45/45**
- **Inventory adjustments + movement ledger ✅ (Session 82 Phase D — `src/lib/inventory-adjustments.ts` + `/api/admin/inventory/{adjustments,movements,layers}` + `/admin/inventory` UI + `verify-inventory.js`):** the single audited path for stock changes outside the existing flows — `applyInventoryAdjustment()` (single-document atomic stockVersion update, exactly-once via unique `sourceRef` movement index + `exists()` pre-check, mandatory reason, insufficient stock → 409 no partial mutation, malformed ids → 400); positive → new FIFO layer at the CONFIRMED cost (never silently supplierPrice), negative → FIFO layers consumed oldest-first, fail-safe on insufficient layers; **post-cutover enforcement** — admin product PUT / supplier stock quick-edit with a changed stock → 400 «…تعدیل…» (non-stock edits untouched; pre-cutover semantics unchanged) so every post-cutover quantity change is ledger-backed; movements API (type/product/date/search/pagination) + layers API (active layers, value = remaining × unitCost); `/admin/inventory` UI (sidebar «انبار»: summary, adjustment form with live preview + confirmation modal, movement history, layer viewer; «در دسترس نیست» instead of fake zeros); **real bug fixed** — the `InventoryMovement` unique partial index (`$ne` in `partialFilterExpression`) never built on MongoDB → changed to `$gt: ""` and rebuilt unique; **E2E singleton-leak fix** — the accounting-wizard journeys now clean the GLOBAL `accountingconfigs` in `afterAll` (root cause of the earlier 3-failure full Playwright run); `verify-inventory.js` 15/15 → **46-suite regression**; unit 11 (548/548 Vitest); Journey 21 E2E 5/5; Playwright chromium 101/102 + mobile 12/12 (single failure = pre-existing Session 81 rich-description external-host flake, independently reproduced); regression **46/46**
- **Expense ledger + net profit ✅ (Session 82 Phase E — `src/models/Expense.js` + `/api/admin/expenses*` ×5 + `/admin/expenses` UI + `verify-expenses.js`):** operating-expense ledger with the approved 10-category enum, status paid|pending|void, and **audited void** (required reason 2–500, rows never deleted — audit trail, voids final, voided pay/update → 400, `EXPENSE_WRITE_LIMIT` 60/15min, validation-before-rate-limit); expenses report (kind `expense` — status/category-enum filters; **totals EXCLUDE voided rows** while they stay visible) + **P&L V2: netProfit = grossProfit − operatingExpenses** (Σ non-void expenses in window; no longer «در دسترس نیست» — a real zero is not a fake zero; `change.netProfit`); Excel expense Summary + detail sheet; sidebar «هزینهها» + list/detail UI with mark-paid + void modal; **mobile-drawer fix** — the new nav item pushed «خروج» below the Pixel-5 fold (whole-aside scroll) and the logout E2E force-click landed on «کاربران»; the drawer now pins the store-link/logout footer while only the nav scrolls; **pre-existing assertions updated for the approved semantics** (verify-reports TEST 7 + admin-reports E2E asserted «net profit unavailable» → netProfit = grossProfit − operatingExpenses); `verify-expenses.js` 10/10 → **47-suite regression**; unit 16 (564/564 Vitest); Journey 22 E2E 2/2 (hermetic ×2); Playwright **116/116** (chromium 104 + mobile 12); regression **47/47**
- **Session 82 Phase C Hardening ✅ (production-readiness stabilization — FIFO rollback + payment-retry integrity):** HIGH-1 checkout SO-failure rollback now restores FIFO layers at the exact snapshot cost (`r.fifoUnitCost` → `restoreStock`) + deletes orphaned SupplierOrders; HIGH-2 payment retry is FIFO-integrated (failed re-reservation restores exact layers; success persists the ACTUAL consumed `fifoUnitCost` on the order items + a per-retry-cycle `sale-…-retry-<token>` compensating movement — never double-written); MEDIUM-4 adjustment crash-recovery (`InventoryMovement.completedAt` — a pending claim is COMPLETED on retry, not falsely idempotent); MEDIUM-5 new-variant stock bypass closed (post-cutover `stock>0` on a new variant → 400 via adjustment); MEDIUM-7 purchase over-payment → 400 instead of silent `Math.min` cap; MEDIUM-1 future cutover date → 400; MEDIUM-6 `sourcing_change` removed from the movement filter UI/API (label + type kept for legacy rows). New tests: verify-fifo 16/16 (forced SO-failure via partial unique index — proves stock+layers+value restored, no sale movement, order+SOs cleaned), verify-payment-retry 16/16 ×2 (purchased fixtures: snapshot refresh 999→1000, partial-failure rollback, concurrent no-double), verify-inventory 16/16 (crash-recovery + new-variant guard), verify-purchases 17/17 (over-pay 400 + no mutation), verify-accounting TEST 3b (future cutover 400). Gates: tsc 0 · check exit 0 · Vitest 570/570 · all accounting suites green · git diff --check clean. Full Playwright + full regression re-run in the regression env (see CHANGELOG).
- **Accounting Excel V2 ✅ (Session 82 Phase F — `src/lib/accounting-v2.ts` + `src/lib/report-matches.ts` + `buildAccountingWorkbook` + `/api/admin/reports/accounting` + `verify-accounting-excel.js`):** the final Session 82 phase — a professional 17-sheet accountant workbook (خلاصه · فروش · سفارشها · اقلام فروش · پرداختها · مرجوعیها · مشتریان · کوپنها · خریدها · اقلام خرید · هزینهها · موجودی · گردش موجودی · لایههای FIFO · بهای تمامشده · سود و زیان · خلاصه حسابداری), all additive and backward-compatible (Session 81 sheets/export untouched). **Server-side data service** `accounting-v2.ts`: per-line order-items with FIFO COGS (`fifoUnitCost` when present, else the immutable supplierPrice snapshot — labelled `cogsSource` fifo|snapshot + `sourceRef`), purchase items (unit cost from PurchaseItem, never current supplierPrice), append-only movement ledger with Persian type labels, **active FIFO layers (Σ remaining × unitCost = inventory value)**, COGS sheet, and an accountant-facing summary with the **valuation-basis label** + historical snapshot note; `null` renders as «در دسترس نیست» — never a fake zero. **Shared `report-matches.ts`** extracted (circular-import fix, byte-identical behavior). Generic report page renders the slug via the standard envelope (rows/totals/summary) — no parallel UI. **XLSX parsed-reconciliation verified:** Σ COGS rows = P&L COGS, Net Sales − COGS = Gross Profit, Gross Profit − Operating Expenses = Net Profit, Σ active FIFO layer values = inventory value, purchase/expense totals, void excluded, 401/403 + export 429. `verify-accounting-excel.js` 10/10 → **48-suite regression**; unit 6 (570/570 Vitest); Journey 23 E2E 2/2 (hermetic ×2, scoped afterAll purge keeps admin-expenses' exact totals isolated); Playwright **117/118** (single failure = the pre-existing external-Parspack rich-description flake, independently reproduced); regression **48/48**; accounting-integrity review — all 9 traceability questions answer YES
- **Profitability report ✅ (Sessions 87–88 — `src/lib/profitability.ts` + `src/components/reports/profitability-view.tsx` + `/admin/reports/profitability`):** the deepest reporting subsystem — line-level aggregation over order items (coupon discounts allocated proportionally by line), 10 KPIs with previous-period comparison, an 8-step waterfall, product/category/expense tables, rule-based diagnostics, financial-health indicators and trend buckets. **Session 87 (commit `c75cba5`) added the foundation; Session 88 completed it:** the export **HTTP 500 is fixed** (the generic summary sheet is now skipped for `profitability` — the branch is self-contained and emits exactly **five sheets: خلاصه سودآوری / آبشار سودآوری / سودآوری محصولات / تحلیل هزینه‌ها / روند**); profitability gained **its own page metadata** (no longer reuses `PNL_META` — title «سودآوری», description, `filters: { preset: true, trendGroup: true }`; the P&L branch is untouched); a **daily/weekly/monthly trend selector** writing `?group=day|week|month`; **correct per-bucket trend boundaries + zero-fill + expense-only periods** (`computeTrendBuckets()`/`assembleTrendBuckets()` — buckets no longer all inherit the overall window; contiguous, non-overlapping, labels matching Mongo's `$dateToString` incl. a real ISO week-year; 3000-bucket defensive cap); and an **accessibility pass** (every `<th>` scoped, sortable headers are real `<button>`s inside `aria-sort` headers, decorative icons `aria-hidden`, charts expose textual alternatives). **Accounting invariants preserved:** COGS = `OrderItem.fifoUnitCost` → historical `OrderItem.supplierPrice` snapshot → `0` (**never** the current `Product.supplierPrice`); **no `category` snapshot added to `OrderItemSchema`, no migration/backfill**; zero changes to P&L/FIFO/inventory/checkout/payment/refund or `report-matches.ts`. **Verified:** tsc 0 · `npm run check` exit 0 (4 pre-existing warnings) · Vitest **623/623** (+20) · new offline **XLSX test** (real workbook built and re-parsed: exactly the five sheets, numeric KPI/waterfall cells — regression-guards the 500) · `verify-reports.js` extended (profitability JSON shape, `netSales − cogs = grossProfit`, `grossProfit − operatingExpenses = netProfit`, trend groups + boundaries, Σ trend = KPI totals, invalid `group` → 400, parsed export workbook). **BLOCKED (not claimed):** `verify-reports.js` over HTTP, Playwright and the full regression could not be run — **MongoDB was unreachable** (the app itself failed with `MongooseServerSelectionError`), so those three layers remain unverified pending a reachable database. Historical category profitability stays intentionally empty (deferred to a schema/migration session).
- **Build:** ✅ Zero TypeScript errors (`npx tsc --noEmit`); CI gates on push/PR via GitHub Actions (Session 60)
...
- User, Product, Order, Category, Supplier, SupplierOrder, Notification, Transaction, File, Brand, Tag, SupplierApplication, CustomerConversation models (13 total; **22 collections as of Session 90** — see the Collections table below for the authoritative inventory)
...
- Multi-language (i18n)
- Testing (Playwright component/e2e expansion, Testing Library component tests)
- Live production rollout (deployment *readiness* is DONE — Session 89: build/start contract, `/api/health`, CI `build` gate, runbook; only the operator-side deploy to Chabokan remains)
- Customer email/SMS order notifications
- Monitoring / error tracking (Sentry)
- Performance (Core Web Vitals) + accessibility audit
- Real-time notifications via WebSocket/Redis (SSE is single-instance in-memory)
...
|------------|-----------|-------|
| **User** | name, phone, passwordHash (optional — Session 62 OTP accounts are passwordless), **tokenVersion** (Session 62 — session-revocation foundation), role, supplier, address, isActive | role: customer/supplier/admin |
| **OtpCode** | phone, purpose (login/register), codeHash (SHA-256 — plaintext never stored in production), attempts, codeConsumedAt (code single-use), consumedAt (login-token single-use), loginTokenHash, loginTokenExpiresAt, devPlaintextCode (SMS_MOCK dev-only), expiresAt | TTL index on expiresAt (2-min lifetime, auto-cleanup); Session 62 SMS OTP rows |
...
| **Order** | customer, items[], totalAmount, shippingAddress, **shipping** (provider, trackingCode, shippedAt, deliveredAt, note), **payment** (status, method, authority, refId, cardPan, paidAt, retryToken), **refund** (reason, refundedAt, refundedBy), status, statusHistory[], **stockRestored**, **smsEvents[] (Session 91 — event `created`/`payment-success`/`shipped`/`delivered`/`refund-completed`, dedupeKey `order:<id>:<event>`, status `pending`/`sent`/`failed`/`unknown`, createdAt) | items[] now include variantId/sku/variantLabel/**image** snapshot; **shipping (Session 57)** = fulfillment metadata captured on shipped/delivered (additive, optional trackingCode); stockRestored prevents double restoration; refund holds refund audit metadata; **smsEvents (Session 91)** = durable business-SMS event markers pushed in the SAME atomic write as the lifecycle transition (durable outbox mirror of `SmsLog.dedupeKey`; terminal states mirror the SmsLog row so a re-run never resends) |
...
| **CustomerConversation** | customer (ref), order (ref), supplierOrder (ref), supplier (ref — from the SupplierOrder, never the body), product (optional ref), category, subject, status (open/pending/resolved/closed), customerUnread, staffUnread, lastMessageAt, lastMessagePreview, lastMessageFrom, messages[] (sender ref, senderRole, text, createdAt), resolvedAt, closedAt | Order-linked support threads (Session 68); **unique partial index `{supplierOrder} where status ∈ (open, pending, resolved)`** — one active conversation per (customer × supplier-order); queue index `{supplier, status, lastMessageAt:-1}` |
| **AccountingConfig** | singleton doc: inventoryInitialized, cutoverDate, initializedBy/At, confirmValuation | Session 82 Phase A — accounting cutover marker; **post-cutover enforcement** keys on `inventoryInitialized` |
| **PurchaseOrder** | number (P-YYYYMMDD-NNN), supplier (ref), purchaseDate, reference, notes, status (draft/ordered/partially_received/received/cancelled), subtotal, discount, additionalCosts, total, paymentStatus (unpaid/partial/paid), amountPaid, items[] (product ref, variantId, quantity, receivedQuantity, unitCost, lineTotal), receipts[] (idempotency key, itemId, quantity, receivedAt, receivedBy), createdBy, cancelledAt/By/Reason | Session 82 Phase B — procurement; **only receiving creates stock + FIFO cost layers + receipt movements** |
| **InventoryMovement** | product (ref), variantId, type (opening_balance/receipt/sale/return_restock/cancellation_restock/purchase_return/adjustment/sourcing_change), quantity (signed), unitCost, totalCost, sourceRef, sourceType (order/purchase/adjustment), description, createdBy, createdAt | Session 82 Phase C/D — **append-only immutable audit ledger**; **unique partial index `{sourceRef} where $type: string, $gt: ""`** (the `$ne` form is invalid in partialFilterExpression — real bug fixed and rebuilt unique); every post-cutover quantity change is ledger-backed |
| **SmsTemplate** | name (unique), type (order_confirmation/shipping_update/tracking_code/delivery_followup/custom), providerTemplateId (optional, digits-only — the SMS.ir pattern id stored in MongoDB, NOT environment variables), variables (SERVER-derived from body placeholders — never client-declared), body (≤500, `{{variable}}` placeholders), isActive, createdBy/updatedBy, timestamps | Session 90 — admin business-SMS templates (Phase 1a); unique name index (E11000 → 409) + `{type, isActive}`; **deliberately parallel to the OTP flow** — the business provider never reads `SMS_IR_TEMPLATE_ID` |
| **SmsLog** | recipient (canonical 09…), user/order/template (optional refs), **templateName snapshot** (survives template deletion), messageType (same enum as SmsTemplate.type), provider (mock/smsir/none), providerMessageId, **status (`sent`/`failed` final outcomes for manual sends — Session 91 order-event rows additionally use `pending`/`sending`/`unknown`)**, **dedupeKey (Session 91 — `order:<id>:<event>`, null on manual sends; unique PARTIAL index where string = the atomic insert gate for order-event rows, no migration needed)**, **attemptCount/claimedAt (Session 91 — claim→send→finalize diagnostics; `claimedAt` is operator visibility, NEVER automatic stale-reclaim)**, error + errorMessage (controlled codes), message (rendered audit copy), sentAt, createdBy actor, timestamps | Session 90 — business-SMS audit trail (Phase 1a; Session 91 added the order-event state machine); query indexes (createdAt · status+createdAt · messageType+createdAt · recipient+createdAt · template); **intentionally NO TTL** (audit rows are never auto-deleted); manual sends keep **no dedupe key** (explicit admin actions) while Session 91 order-event rows carry the deterministic `order:<id>:<event>` key gated by the unique partial index |
...
- **Model** — `OtpCode` (new collection `otpcodes`): `phone`, `purpose`, SHA-256 `codeHash`, `attempts`, `codeConsumedAt` (code) vs `consumedAt` (login token — the split keeps each single-use independently; the E2E caught the original verify-consumption bug), `loginTokenHash`/`loginTokenExpiresAt` (TTL-aligned), `devPlaintextCode` (mock-only), TTL index. `User.passwordHash` optional (default null) + `tokenVersion` (default 0). **Model change ⇒ dev-server restart required** (done).
- **`src/lib/otp.ts`** — pure crypto/constants: `normalizePhone` (+98/0098 → 09), `generateOtpCode` (crypto.randomInt), SHA-256 hashing, expiry/consumed helpers, TTL/cooldown/attempt constants, `smsIrMobile`.
- **`src/lib/sms.ts`** — adapter-first `sendOtp`: **mock** (dev + `SMS_MOCK=1` — code logged + `devPlaintextCode`), **sms.ir** (production, active ONLY with `SMS_IR_API_KEY` + `SMS_IR_TEMPLATE_ID`; `POST https://api.sms.ir/v1/send/verify`, `x-api-key`, `{mobile, templateId, parameters}`), **none** → controlled `SMS_NOT_CONFIGURED` (request API → 503). Never throws; injectable env/fetch for unit tests. `.env.example` documents the future sms.ir vars only.
- **Routes** — `POST /api/auth/otp/request` (validation-before-limiter, per-phone 5/15min + per-IP 15/15min, 60s resend cooldown → 429+Retry-After, register 409 on existing, **login anti-enumeration**: unknown phone → same `200 {sent:true}` with nothing created/sent) · `POST /api/auth/otp/verify` (per-phone 5/15min, 5-attempt lock, register creates passwordless customer / login requires account, issues one-time `loginToken`) · `GET /api/auth/otp/dev-last` (dev/mock-only code reader — 404 elsewhere).
- **NextAuth** — CredentialsProvider `loginToken` branch: atomic claim (`updateOne({_id, consumedAt: null})`) → replay-proof; password branch untouched; `jwt`/`session` callbacks now carry `tokenVersion` + `phone` (the declared `session.user.phone` contract is now real — additive); `next-auth.d.ts` augmented.
- **UI** — login/register method toggles (password default), shared `OtpPanel` (request → verify → signIn) + `OtpCodeInput` (numeric, one-time-code); toggle inactive-tab contrast → zinc-600/400 (axe 4.39→7+:1).
- **Verification** — `tsc` zero errors; `npm run check` passes; Vitest **183/183** (158 + 25 new otp/sms tests); Playwright **30/30** (chromium 25 incl. 6 axe scans + Journey 12 OTP; login/register axe scans clean; customer-login «ورود» locator → `exact: true`); `scripts/verify-otp.js` **11/11** (mock seam, request/cooldown/anti-enumeration, wrong+correct code, token exchange + replay rejection, OTP-for-password-user, passwordless-can't-password-login + password-compat, per-IP cap 429); full regression **34/34 PASS**; code review approved.
...
### 🟢 Minor
4. ~~**Middleware deprecation:** Next.js 16 warns that `middleware` convention is deprecated in favor of `proxy`.~~ **RESOLVED (Session 89)** — migrated to `src/proxy.js` (export renamed `middleware` → `proxy`, matcher/redirects/RBAC byte-identical, git rename `R081`); the build warning is gone and the route-guard E2E passes 25/25.
...
5. **Unit/E2E tests + CI now exist** (Session 58 Vitest 152/152 + Session 59 Playwright 21/21 + Session 60 GitHub Actions `ci.yml`) — remaining gap: component tests (Testing Library) and actually running the pipeline on GitHub (needs a repo push + the optional `regression.yml` secrets for the nightly 34-suite run).
...
## Performance & Accessibility (Session 61)

- **Storefront `next/image` migration (12 files / 17 images)** — product-card, quick-categories, product detail gallery + thumbnails, image-lightbox (zoom + thumbs), cart, checkout, order-invoice, hero-carousel, campaign-banner, gift-collections, supplier-card, supplier detail logo. `fill` + `relative` aspect containers (no layout shift), explicit `sizes`, `loading="eager"` only above the fold, all `onError` placeholder fallbacks preserved. **Zero behavior change.**
- **`next.config.ts` `images.remotePatterns`** — additive allowlist from env at config-load: `LIARA_ENDPOINT` hostname (fallback `c589564.parspack.net`), `NEXT_PUBLIC_APP_URL` when set, localhost (dev); optimizer stays enabled (AVIF/WebP + srcset).
- **Accessibility audit fixes (in-scope only)** — `search-suggestions.tsx` (`aria-selected` on listbox options), `image-lightbox.tsx` (labelled backdrop + zoom `<button>`s, tab order preserved; retry button hidden for guard-blocked URLs), `storefront-header.tsx` (Persian `aria-label`s on icon-only links), `hero-carousel.tsx` (`inert` on hidden slides — axe `aria-hidden-focus`), `ui/badge.tsx` (success/warning contrast → WCAG AA, -700 shades); `mobile-drawer` verified already-compliant.
- **`isAllowedImageSrc` fail-safe guard** (`src/lib/utils.ts`, +5 unit tests) — next/image throws at render on unconfigured hosts (native `<img>` degraded to a broken image); the guard is applied across all 12 storefront image components so unconfigured URLs fall back to placeholders instead of crashing a section (dev DB's leftover `example.com` images crashed homepage + catalog under the migration).
- **axe-core E2E gate** — `@axe-core/playwright` devDep + `tests/e2e/accessibility.spec.ts` (Journey 11): scans homepage/catalog/product-detail/supplier-detail/login/register (desktop chromium), asserts **zero serious/critical** wcag2a/2aa/21a/21aa violations; runs inside `npm run e2e` → inside the Session 60 `ci.yml` e2e gate; `OUT_OF_SCOPE_RULE_IDS` documents any future out-of-scope findings.
- **`PERFORMANCE.md` (new)** — image pipeline, allowlist, loading strategy, the admin/supplier native `<img>` debt (deliberately out of scope), axe gate.
- **Verification** — `tsc` zero errors, `npm run check` passes (scoped eslint 0 errors, 4 pre-existing warnings; `src/components` out of gate scope — `image-lightbox` carries 4 pre-existing react-hooks errors on untouched effects, documented debt), Vitest **158/158** (152 + 6 new), Playwright **27/27** (chromium 22 incl. 6 axe scans + mobile 5, zero serious/critical violations on all six scanned pages); regression unaffected (zero API/server/model changes); no dev-server restart needed.
...
- ✅ 20/20 verification tests; `npx tsc --noEmit` zero errors; regressions green (payouts 17/17, refund 12/12, payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
2. **Critical destructure misalignment** — a duplicate `discountSummary` aggregation in the `Promise.all` array vs a single name in the destructure shifted every binding from `topCoupons` onward and 500'd every request (orderStatuses got a number → `.map` crash). Route rewritten with exactly 14/14 aligned bindings.
...
`npx tsc --noEmit` → **zero errors** ✅; full regression green across all 19 suites (sequential — the one transient failure was a Zarinpal 502 during the post-restart run; re-run passed 14/14). Code-reviewer approved (4 rounds).
...
- ✅ 18/18 verification tests; `npx tsc --noEmit` zero errors; regressions green (wishlist 14/14, reviews 20/20, notifications 18/18, supplier-replies 21/21, refund 12/12, payouts 17/17, payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
- ✅ 21/21 verification tests; `npx tsc --noEmit` zero errors; regressions green (reviews 20/20, notifications 18/18, wishlist 14/14, refund 12/12, payouts 17/17, payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
- ✅ 18/18 verification tests; `npx tsc --noEmit` zero errors; regressions green (wishlist 14/14, reviews 20/20, payouts 17/17, refund 12/12, payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
- ✅ 14/14 verification tests; `npx tsc --noEmit` zero errors; regressions green (reviews 20/20, payouts 17/17, refund 12/12, payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
- ✅ 17/17 verification tests; `npx tsc --noEmit` zero errors; regressions green (refund 12/12, payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
- ✅ 12/12 verification tests; `npx tsc --noEmit` zero errors; regressions green (payment-retry 14/14, variant-polish 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
...
- ✅ 14/14 verification tests; `npx tsc --noEmit` zero errors; regressions green (payment-retry 14/14, pagination 24/24, variants 16/16, upload-repro 15/15)
