...
| **Order** | customer, items[], totalAmount, shippingAddress, payment, status, statusHistory[], **smsEvents[] (Session 91 — durable business-SMS event markers: event/dedupeKey/status/createdAt)** | items = snapshots at purchase time; smsEvents markers are pushed in the SAME atomic write as the lifecycle transition (durable outbox; terminal states mirror SmsLog) |
...
| **SmsLog** | recipient (canonical 09…), user/order/template (optional refs), templateName snapshot (survives template deletion), messageType, provider (mock/smsir/none), providerMessageId, status (**manual sends: `sent`/`failed` written AFTER the provider returns — Session 91 order-event rows: `pending`/`sending`/`sent`/`failed`/`unknown` state machine**), **dedupeKey (Session 91 — `order:<id>:<event>`, null on manual sends; unique PARTIAL index = the atomic insert gate)**, attemptCount/claimedAt (claim diagnostics — never used for automatic stale-reclaim), error info, message (rendered audit copy), sentAt, createdBy | Business-SMS audit trail (Session 90 Phase 1a); query indexes; **intentionally NO TTL**; manual sends have no dedupe key (explicit retries) while Session 91 order-event rows are deduped by the unique partial index + CAS state machine (`src/lib/sms-order-events.ts`) |

### SMS Architecture Note (Session 90 + Session 91)
...
**Order-event automation (Session 91):** `src/lib/sms-order-events.ts` sits on top of the business service and is the single claim→send→finalize path for the five order lifecycle events. Durable `Order.smsEvents` markers are committed in the SAME atomic write as each lifecycle transition; the deterministic `order:<orderId>:<event>` dedupe key + the unique partial index on `SmsLog.dedupeKey` gate row creation; single-document CAS claims make concurrent triggers send at most once (standalone MongoDB — no multi-document transactions, no worker/cron/queue). A pending marker whose SmsLog is already `sent` repairs the marker with ZERO provider calls; stale `sending` rows are never auto-reclaimed and `unknown` outcomes are terminal for automation (SMS.ir has no idempotency key, so a retry could double-send). Disabled provider / missing-inactive template ⇒ no provider call, marker stays pending. `fireOrderSmsEvent()` never throws, so an SMS failure can never break order/payment/shipping/refund.
