...
## Session 91 (September 2026) — Business SMS Order-Event Automation (پیامک رویدادهای سفارش)

### What shipped — the five real lifecycle events, wired to the Session 90 business-SMS service
- **ORDER_CREATED · PAYMENT_SUCCESS · ORDER_SHIPPED · ORDER_DELIVERED · REFUND_COMPLETED** — automatic business SMS on the actual order lifecycle: `POST /api/checkout` (order created, `pending_payment`), `GET /api/payment/verify` (atomic payment `pending→paid`, order → `processing`), the admin order PUT (atomic `confirmed→shipped` storing `shipping.shippedAt`/`trackingCode`, and `shipped→delivered` storing `shipping.deliveredAt`), and `POST /api/admin/orders/refund` (atomic payment `paid→refunded`). Templates resolve by deterministic application-level NAME (`order-created` / `payment-success` / `order-shipped` / `order-delivered` / `refund-completed` — multiple templates may share a type; the `SmsTemplate.type` enum is only the SmsLog `messageType` category); `providerTemplateId` always comes from the stored template document, never a route.
- **New `src/lib/sms-order-events.ts`** — the durable claim→send→finalize processor on top of `sms-business.ts` (provider calls remain strictly behind `sendBusinessSms`; `normalizePhone` reused read-only from the OTP module).

### Durability + idempotency (no transactions, no worker)
- **Durable `Order.smsEvents` markers** (additive embedded array: `event`/`dedupeKey`/`status`/`createdAt`) — each marker is pushed in the SAME atomic write that commits the transition (embedded in `Order.create` via a pre-generated `_id` for ORDER_CREATED; `$push`ed inside the pending→paid / status-claim / paid→refunded `findOneAndUpdate` for the rest), so the fact that an SMS is owed is durable the instant the transition commits.
- **Deterministic dedupe keys** — `order:<orderId>:created|payment-success|shipped|delivered|refund-completed`; a unique PARTIAL index on `SmsLog.dedupeKey` (string only) is the atomic insert gate (manual sends keep `dedupeKey: null` and their intentional-retry semantics; no migration needed).
- **SmsLog state machine** — status enum extended to `pending → sending → sent|failed|unknown` (+ `attemptCount`, `claimedAt` diagnostics): insert gate (exactly one row per logical event, losers E11000 → read-and-handle) → send gate (single-document CAS `findOneAndUpdate` on the exact source state; losers' filters no longer match → no second provider call) → finalize (CAS `sending →` terminal, written only by the claim owner).
- **Synchronization rule (crash between send and marker update)** — pending Order marker + existing `sent` SmsLog ⇒ ZERO provider calls and the marker is repaired to `sent` (idempotent on repeated processing).
- **No automatic stale `sending` reclaim** (SMS.ir has no idempotency key — a reclaim could double-send; `claimedAt` is operator visibility only). **`unknown` (network timeout/ambiguous outcome) is terminal for automation** — never auto-retried; explicit manual CAS retry only. Exactly-once EXTERNAL delivery is documented as NOT guaranteed (the intrinsic uncertainty window is documented in the module header).
- **Provider/template safety** — confirmed provider rejection → `failed`; network/timeout → `unknown`; disabled/unconfigured provider (`resolveBusinessSmsProvider() === "none"`) → no provider call, no SmsLog row, durable marker stays pending; missing/inactive/empty-body template → `template_unavailable`, no provider call, marker stays pending so a later activation is still processed.
- **Lifecycle isolation** — `fireOrderSmsEvent()` never throws; every route fires it best-effort AFTER the committed transition; an SMS failure can never fail order/payment/shipping/refund. Phones/names are always server-derived (auth token / customer record), never client input.
- **No MongoDB multi-document transactions, no background worker/cron/queue** — standalone-MongoDB-safe; `scripts/verify-sms-order-events.js` is a test utility, NOT a production reconciler (pending markers survive crashes for a future worker/admin recovery path).

### Scope boundaries (untouched by design)
- **OTP/auth/payment/stock/refund/accounting/FIFO behavior intentionally outside Session 91 scope** — the four lifecycle routes only gained the marker `$push` (inside the existing atomic claim) and the post-commit fire-and-forget; all claim filters, status flows, stock reservation/rollback, refund accounting and coupon semantics are byte-identical. All 8 OTP-protected files (`sms.ts`, `otp.ts`, all OTP routes, `auth.js`, `password-reset/complete`, `OtpCode.js`) + every OTP test/verify suite verified unchanged; OTP rate-limit keys/limits untouched. No schema migration/backfill (the embedded array + partial index build lazily).

### Verification + shipping
- **`scripts/verify-sms-order-events.js` 9/9 PASS** over the real dev server + real MongoDB (template CRUD for the five names; real customer register→login→checkout writing the ORDER_CREATED marker atomically with `order:<id>:created`; deterministic distinct keys; ≤1 row per key; unique-index E11000 enforcement; sent-row singularity; failed/unknown states persisted; `otpcodes` count unchanged; fixture self-cleanup).
- **`tests/unit/sms-order-events.test.ts` 18/18** — hermetic provider-call-count proofs: concurrent first-time triggers → exactly one send (E11000), concurrent retries of failed/unknown rows → exactly one CAS winner, repeated processing of sent rows → zero resends, sent-repair, no stale-`sending` reclaim, provider mapping (API_ERROR→failed, NETWORK_ERROR→unknown), disabled provider → no row/no send, missing/inactive template → untouched, no-phone → not_sent, failure containment.
- **Full gates + pipeline:** tsc 0 · `npm run check` exit 0 (same 4 pre-existing warnings) · Vitest **687/687** · `git diff --check` clean · **CI run `35613757314` all four jobs PASS** (Unit/Build/Static/E2E) · **Deploy Production run `35614725034` PASS** · production smoke: `https://marloo.shop` 200 · `/api/health` 200 `db:up` · PM2 online · no Session 91 runtime errors — **production runs exactly `4041927`**.

## Session 90 Phase 1 (September 2026) — Admin Business SMS Management (مدیریت پیامک‌ها) — Phases 1a + 1b + 1c

### The approved scope (order-event automation, real SMS.ir business-pattern activation, bulk campaigns and scheduling are explicitly DEFERRED)

### OTP isolation contract — the foundation of the whole phase
- **Business SMS is a deliberately PARALLEL module, never an extension of the OTP stack.** New `src/lib/sms-business.ts` imports nothing from `src/lib/sms.ts` and shares no resolver: the business provider resolver (`resolveBusinessSmsProvider`) reads `SMS_IR_API_KEY` only and **never reads `SMS_IR_TEMPLATE_ID`** (that template belongs to the OTP flow) — unit-tested by an explicit isolation block that re-asserts the OTP provider's own gate, mock seam and `/v1/send/verify` payload contract with business flags set.
- **The following OTP-sensitive files remain byte-for-byte untouched:** `src/lib/sms.ts`, `src/lib/otp.ts`, `src/app/api/auth/otp/request/route.js`, `src/app/api/auth/otp/verify/route.js`, `src/app/api/auth/otp/dev-last/route.js`, `src/lib/auth.js`, `src/app/api/auth/password-reset/complete/route.js`, `src/models/OtpCode.js`, plus every OTP unit/E2E test and `scripts/verify-{otp,password-reset}.js`. Verified after implementation by `git diff` over each path (all empty) and by re-running the OTP suites green (below).
- **Existing OTP rate-limit keys/limits untouched** — the new business limits use independent `sms-send:` / `sms-template-write:` key namespaces (see below).

### Phase 1a — models + the business-SMS service
- **New `SmsTemplate` model** (`src/models/SmsTemplate.js`): unique `name` (unique index → E11000 mapped to 409), business message `type` enum (`order_confirmation` / `shipping_update` / `tracking_code` / `delivery_followup` / `custom`), optional **digits-only** `providerTemplateId` (the SMS.ir pattern id, stored in **MongoDB — not** environment variables; may be empty while the system runs mock-first), **server-derived** `variables` (always re-derived from the body placeholders — a client can never declare a variable the body does not contain), `body` ≤ 500 chars with `{{variable}}` placeholders, `isActive`, `createdBy`/`updatedBy` audit refs, timestamps; index `{ type, isActive }`.
- **New `SmsLog` model** (`src/models/SmsLog.js`): one immutable row per send attempt — canonical `recipient` (09…), optional `user`/`order`/`template` refs, **`templateName` snapshot** (survives template deletion, keeps history readable), `messageType`, `provider` (`mock`/`smsir`/`none`), `providerMessageId`, final `status` (`sent`/`failed` — the row is written AFTER the provider returns, so no dangling "queued" state), controlled `error` code + `errorMessage`, rendered `message` (audit copy, never OTP material), `sentAt`, `createdBy` actor, timestamps. Query indexes (`createdAt`, `status+createdAt`, `messageType+createdAt`, `recipient+createdAt`, `template`). **Intentionally NO TTL** (an audit record is never auto-deleted) and **intentionally NO dedupe key** (sends are explicit manual admin actions; order-event dedupe belongs to the deferred automation phase).
- **New `src/lib/sms-business.ts`** — mock-first, kill-switched, injectable: pure `renderTemplate()` (fail-closed — missing variables are reported and left intact; empty/whitespace values count as missing; output capped at 500 chars) + `extractTemplateVariables()` (first-occurrence, deduplicated); `resolveBusinessSmsProvider()`: **mock** (dev + `SMS_MOCK=1`, same dev-seam UX as OTP but a separate adapter, no network) → **smsir** (ONLY when the explicit kill-switch `SMS_BUSINESS_ENABLED=1` AND `SMS_IR_API_KEY` are set; documented `POST /v1/send/bulk` text-SMS endpoint, `x-api-key` auth — the same account-level key the OTP module uses) → **none** (default). Both adapters take injectable `env`/`nodeEnv`/`fetchImpl` (the `sms.ts` convention) so tests are hermetic; failures are CONTROLLED returned values, never thrown, so a business-SMS failure can never crash an unrelated admin request.

### Phase 1b — admin APIs + verification suite
- **`/api/admin/sms/templates`** (GET/POST/PUT/DELETE, admin-only): list (newest first, `?active=true`), create (duplicate name → 409; client-declared `variables` ignored), partial update (body edits re-derive `variables`; unknown id → 404, malformed → 400), delete (SmsLog rows keep their snapshot). Write verbs rate-limited `SMS_TEMPLATE_WRITE_LIMIT` **30/admin/15min**.
- **`/api/admin/sms/send`** (POST, admin-only): the manual business send. Exactly one of `templateId` / `message` (both/neither → 400); recipient **normalized server-side** (`normalizePhone` reused read-only + `^09\d{9}$` gate); template state loaded from the DB (a request body can never inject a `providerTemplateId` or bypass a stored template); **inactive template → 400 refusal (no SMS when disabled, no log row)**; fail-closed variable validation (missing → 400 with the named list; extra keys → 400); free-form messages sanitized + bounded (500); sender identity ALWAYS the authenticated admin token (never the body); optional `userId`/`orderId` links are validated ObjectIds (unknown user → 404). The provider call never throws and **every attempt — success or failure — is persisted to SmsLog**; failures return a stable error code only (no provider internals, no secrets). Rate-limited `SMS_SEND_LIMIT` **10/admin/15min** — independent namespaces `sms-send:` / `sms-template-write:`; OTP keys untouched.
- **`/api/admin/sms/logs`** (GET, admin-only): paginated audit trail (Session 76 pagination helpers) with `status` / `messageType` / `templateId` filters + escaped recipient search.
- **Models + rate-limiter additions are additive only** — no existing model, route or limiter config was modified.

### Phase 1c — admin UI
- **New `/admin/sms` page** («پیامک‌ها» in the admin sidebar, following the `/admin/suppliers` tab pattern) with three tabs: **قالب‌ها** (template CRUD — the UI edits name/type/body/providerTemplateId only; variables always come back server-derived; activate/deactivate; delete), **گزارش ارسال** (SmsLog list with status/type filters + recipient search + pagination), **ارسال دستی** (template-based with per-variable inputs, or free-form; client-side can-send gating mirrors the server rules; every send result toasted and audited). React Query hooks in `src/hooks/use-admin-sms.ts`; shared types (`AdminSmsTemplate`, `AdminSmsLog`, …) in `src/types/index.ts`.

### Production safety — the kill-switch
- **Production business SMS is DISABLED BY DEFAULT.** The default provider is `none`: with the current production env (OTP fully configured, no `SMS_BUSINESS_ENABLED`), `resolveBusinessSmsProvider()` returns `none` and every send returns the controlled `SMS_BUSINESS_DISABLED` error — proven by a unit test that reproduces the exact current production env shape. Real business sending requires the explicit, reviewable `SMS_BUSINESS_ENABLED=1` **plus** the existing `SMS_IR_API_KEY`. **No environment variable was changed or added to any environment in this session** — the switch is documented in the module header and remains unset.

### Verification
- **`scripts/verify-sms.js` — 17/17 real-API PASS (×2 runs incl. a re-run isolation proof)**, wired into `scripts/run-regression.js` → **51 suites** (`.github/workflows/regression.yml` count trail updated to 51). Coverage: unauthenticated 401 (all three surfaces) · customer 403 (all three) · template create 201 with **server-derived variables** (client-declared ignored) · duplicate name 409 · invalid payloads 400 (bad type / non-numeric provider id / oversized body) · update re-derives variables + unknown id 404 / malformed 400 · inactive-template send refused 400 with **no log row** · template send (mock) → 200 + SmsLog sent row (rendered text, templateName snapshot, actor audit, messageType mirror) · missing/extra variable 400 · free-form sanitized send · phone + ObjectId validation · exactly-one-of rule · deterministic rate-limit boundary (**10 OK → 11th 429 + Retry-After**, fixture-key isolated) · logs filters/pagination envelope · template delete with snapshot durability · **OTP isolation (`otpcodes` count unchanged across the whole suite)**.
- **Full gates:** `git diff --check` clean · `npx tsc --noEmit` **0 errors** · `npm run check` exit 0 (same 4 pre-existing warnings, none in new files) · Vitest **669/669** (38 files; +25 new `sms-business` tests incl. the OTP-isolation block) · admin-sms Playwright E2E **3/3** (UI create→send→log row, customer 403 server-side RBAC, inactive-template server refusal) · **OTP suites green after the change:** `verify-otp.js` **13/13**, `verify-password-reset.js` **22/22**, OTP E2E (`otp-login` + `password-reset` + `session-security`) **9/9**.
- **Database impact:** none beyond Mongoose's lazy collection/index creation on first use — **no migration, no backfill, no TTL index**; the E2E teardown sweeps `smstemplates`/`smslogs` by run prefix (verified 0 leftovers).

## Session 89 (September 2026) — Production Readiness: Next 16 Proxy, Health Endpoint, Deployment & Build Gate (وضعیت تولید)

### The approved scope (Phases 0–8; Sentry/monitoring explicitly EXCLUDED for a future session)

### Next.js 16 `middleware` → `proxy` migration (behaviour-preserving)
- **`src/middleware.js` → `src/proxy.js`** — Next.js 16 renamed the file convention, so every build emitted `⚠ The "middleware" file convention is deprecated. Please use "proxy" instead.` The file was renamed and its export changed from `middleware` to `proxy`; the `matcher` (`['/admin/:path*', '/supplier/:path*']`), both redirect targets (`/login` for anonymous, `/` for a wrong role) and every RBAC decision are **byte-identical** — git records it as a rename (`R081`). Zero changes to `auth.js`, the CredentialsProvider, OTP, password login, `auth-utils`, `token-version`, the User model or any API route.
- **Verification:** the deprecation warning is gone from a clean build, and the route-guard E2E coverage that exercises the proxy end-to-end passes **25/25** (`logout` — incl. post-logout `/admin` + `/supplier` → `/login` guards — `session-security`, `customer-login`, `otp-login`; desktop + mobile projects).
- **Docs that describe the guard were repointed** (`AUTHENTICATION.md`, `RBAC.md`, `AI_CONTEXT.md`, `ARCHITECTURE.md`) so no current-state doc still names a file that no longer exists. Historical session narratives in `CHANGELOG.md`/`NEXT_SESSION.md` were deliberately left as written.

### Production build (Phase 1) — no blockers found
- **`npm run build` exits 0** on the committed tree: compiled successfully, TypeScript clean, **123 static pages** generated. It is also **database-independent** — verified by building with `MONGODB_URI=mongodb://127.0.0.1:1/nope`, which also exits 0 (prerendered pages do not touch Mongo, and `/api/health` is `force-dynamic`). The only build warning was the middleware deprecation, removed by the migration above.
- Build-time external dependency documented: `next/font/google` (Geist + Vazirmatn) fetches fonts during the build and then self-hosts them from `.next/static/media` — a build machine with no outbound network cannot complete a build (runtime unaffected).

### Deployment configuration (Phase 3) — the platform model, then the minimum config
- **The deployment model was established from the platform's own documentation, not assumed.** Chabokan is used by this project for **MongoDB hosting** (`services.irn2/services.irn9.chabokan.net`), and its cloud-hosting **"NextJs" service** is a *managed app host*: it runs the project's own `npm install` → `npm run build` → `npm start` scripts, expects the app on port **3000** (dashboard-adjustable), takes Node 14–24, and holds all configuration in its **service dashboard** (env vars, Node version, ports). Deploy is via `chabok deploy` (CLI), git, FTP or the file manager. There is therefore **no platform manifest to add** — no `vercel.json` and no Dockerfile were created, because the selected hosting path reads neither.
- **`package.json` gains exactly one production fact:** `"engines": { "node": ">=20.9.0" }` — Next.js 16's own floor. Chabokan offers Node 14/16/18, either of which would break the app at runtime; the `engines` field documents the requirement at the contract level instead of relying on the dashboard being set correctly. `build`/`start` were already correct (`next build` / `next start`, and `next start` honours `PORT`).
- **No secrets** are hard-coded anywhere in the deployment path; the env contract is unchanged (`.env.local` locally, the service dashboard in production).
- **Verified by actually running it**, not by inspection: `scripts/verify-deployment.js` boots the real production server and asserts the build artifact, the boot, `/api/health`, a DB-backed route, the production static-asset cache signature, non-leakage of secrets into the server log, and that no legacy MongoDB target is in use — **7/7 PASS**.

### Health / readiness endpoint (Phase 4) — `GET /api/health`
- **New `src/lib/health.ts`** (pure contract: `DbReadiness`, `HealthStatus`, `isHealthy()`, `healthStatusCode()`, `buildHealthPayload()`) and **`src/app/api/health/route.ts`** (`export const dynamic = "force-dynamic"`).
- **Semantics:** public/unauthenticated, never cached (`Cache-Control: no-store`), non-mutating (its only query is the MongoDB `ping` command), and **bounded to 4 s** — the connection attempt is raced against a budget because `dbConnect()` inherits the driver's 30 s `serverSelectionTimeoutMS`, which is far too slow for a load balancer. Returns **200** `{status:"ok",db:"up",uptimeSeconds,latencyMs,timestamp}` when ready and **503** `{status:"degraded",db:"down",…}` when the database is unreachable — never a false 200. The body is a closed field set and deliberately carries **no** connection string, host, database name, credential or error message (a driver error can embed the URI, so it is swallowed, never logged or returned).
- **Both paths verified live:** 200/`db:up` against the real database, and — with the app pointed at an unreachable Mongo — **503 in 4077 ms** with `{"status":"degraded","db":"down","latencyMs":4000,…}`, confirming the bound and the honest status.
- **New `tests/unit/health.test.ts`** (8 tests) pins the status-code mapping, the payload shape, metric normalisation and the no-secret property; **new `scripts/verify-health.js`** (9 real-HTTP probes) covers the unauthenticated 200, the closed field set, `no-store`, the latency budget, the no-leak scan, idempotence, and cross-checks `db:up` against a DB-backed route so the readiness claim can never be cosmetic.

### CI production-build gate (Phase 5)
- **`.github/workflows/ci.yml` gains a fourth merge-gating job, `build`** — `npm ci` + `npm run build` on every push/PR, alongside the existing `static` / `unit` / `e2e` jobs (which were left untouched). It is **hermetic**: `next build` needs no database (proved above) and no real secrets, so the job defines CI-only placeholders (`NEXT_PUBLIC_APP_URL`, `NEXTAUTH_URL`, `NEXTAUTH_SECRET`, `MONGODB_URI`) that satisfy the two fail-fast guards the build evaluates (`src/lib/constants.ts`, `src/lib/env.ts`). The workflow's stale "`next build` is NOT a gate" comment was corrected, including its one remaining real constraint (the Google Fonts fetch). No Sentry, no new services, no test-semantics changes.

### Deployment verification script (Phase 6)
- **New `scripts/verify-deployment.js`** — build artifact exists (`.next/BUILD_ID` + `.next/server`, with an opt-in `--build`), the app **boots from the production build** (the `start` script is asserted to be `next start`, then that binary is launched on its own port, default 3100 via `VERIFY_DEPLOY_PORT`), `/api/health` → 200 + `db:up`, a **production-vs-dev discriminator** (static chunks served `immutable`/long-max-age, which `next dev` does not do), a DB-backed route serving, **no secret value appearing in the captured server log** (URI, password, host, access/secret keys, NextAuth secret, merchant id, bot token — compared programmatically and never printed), and **no legacy MongoDB target** (a negative guard on the decommissioned `irn2` host plus behavioural proof: a dead host cannot report `db:up`). It deliberately stays **out of the 49→50-suite regression**, because that runner drives a dev server on :3000 while this suite drives a production build on its own port; the release checklist in `README.md` runs both.

### Documentation + a credential-hygiene finding (Phase 7)
- **`README.md` replaced** — the create-next-app boilerplate is gone; the file is now a practical runbook: requirements (Node ≥ 20.9), local setup, admin bootstrap, the environment-variable contract, scripts, **production build & the Chabokan deployment model**, the health probe and what its responses mean, the verification layers, a rollback/checkpoint procedure (commit-level rollback; **no migrations**, so a code rollback needs no data rollback; never hand-edit FIFO/COGS snapshots) and known constraints.
- **⚠️ Credential-hygiene finding + fix — `.env.example` contained the real environment.** The file was correctly gitignored (`.gitignore:.env*`), so **nothing leaked into git history**, but on disk **11 of its 12 values were byte-identical to the live `.env.local`** (Mongo URI, NextAuth secret, Telegram token, Zarinpal merchant id, Liara access/secret keys…). Since `.env.example` is the file that gets committed, shared and pasted into tools, it was replaced with a **placeholders-only template** (real values now differ on every secret; the three remaining matches are the non-secret dev defaults `NEXTAUTH_URL`, `NEXT_PUBLIC_APP_URL`, `LIARA_REGION`) and given an explicit "this file must contain no real secrets" warning. `.gitignore` gained a single `!.env.example` negation so the **sanitized** template is committable while `.env.local` and every other `.env*` file stay ignored. The template also now documents deployment/runtime (`PORT`, `NODE_ENV`, the `NEXT_PUBLIC_APP_URL` build guard, the health probe) and records that `JWT_SECRET` is read by **no** source file (NextAuth uses `NEXTAUTH_SECRET`).
- **`.gitignore`** also keeps the existing build/artifact ignores (`.next`, `build`, coverage, Playwright reports, `.env*`).

### Test-harness integration
- **`scripts/run-regression.js` — now 50 suites** (`verify-health` added second, after the hermetic `verify-db-reconnect`): the health suite is cheap, needs only the already-running server + database, and gives the endpoint regression protection. `.github/workflows/regression.yml`'s title and its per-session count trail were updated to 50, with a note that the production-build/deployment path is a *separate* gate (it needs a build and its own port) and is intentionally not part of the dev-server-driven sequence.

### Tests & verification
- tsc 0 · `npm run check` exit 0 (0 errors, the same 4 pre-existing warnings) · Vitest **631/631** (623 + 8 new health tests) · route-guard E2E subset **25/25** · `scripts/verify-health.js` **9/9** · `scripts/verify-deployment.js` **7/7** (real production build booted on :3100) · `/api/health` negative path **503 @ 4077 ms** with an unreachable database.

### Environment note (carried into this session)
- The stale OS-inherited `MONGODB_URI` that pointed at the decommissioned server was resolved by restarting the host process, so the app now loads `.env.local` normally. All Session 89 work ran against the **new** database with **no env override, no `unset`, no mocks**; the health/deployment probes additionally assert that no legacy target is contacted.

### Deferred / not done
- **Monitoring & error tracking (Sentry)** — explicitly out of scope for Session 89.
- **Live rollout** — this session makes the app deployable and verifiable; pushing it to the Chabokan service is an operator action (runbook in `README.md`).

## Session 88 (September 2026) — Profitability Report Completion (سودآوری)

### The approved scope (completion of the Session 87 foundation — UI + export + trend only; no schema/migration)
- **Profitability Excel export fixed (was a hard HTTP 500)** — `buildReportWorkbook()` ran the generic `writeSummarySheet()` before the profitability switch, and the profitability payload has no generic `summary` envelope → `Cannot read properties of undefined (reading 'orders')`, so `GET /api/admin/reports/profitability/export` 500'd. The generic summary sheet is now skipped for `profitability` (the same guard already used for `purchases`) and the profitability branch is **self-contained**, writing exactly five Persian sheets with the existing workbook helpers (`kpi`-style value handling, `writeDetailSheet`, `flattenRows`, `MONEY`, `PERCENT`, `INTEGER`): **خلاصه سودآوری** (KPI name/value; money and percent in separate columns so each cell keeps its own number format) · **آبشار سودآوری** (all waterfall steps + value + cumulative + type) · **سودآوری محصولات** (columns preserved verbatim from the foundation — the existing product export is unchanged) · **تحلیل هزینه‌ها** (amount + both % shares, non-void only) · **روند** (group, from, to, netSales, COGS, grossProfit, operatingExpenses, netProfit). The conditional category-sheet logic is **preserved** for the future category-snapshot work — `categories` is always `[]` today, so exactly those five sheets are emitted.
- **Profitability got its own page metadata** (`src/app/admin/reports/[report]/page.tsx`) — it previously reused `PNL_META`, so the page rendered the P&L heading/description («سود و زیان») and the trend-group filter was unreachable. New `PROFITABILITY_META`: title «سودآوری», profitability-specific description, `filters: { preset: true, trendGroup: true }`. Meta selection is `isPnl ? PNL_META : isProfitability ? PROFITABILITY_META : REPORT_META[report]` — **the P&L branch is untouched** and still uses `PNL_META`.
- **Trend Group UI** (`src/components/reports/report-filters.tsx`) — a segmented control rendered **only** when `meta.filters.trendGroup`, writing the existing `?group=` param (`روزانه`→`day`, `هفتگی`→`week`, `ماهانه`→`month`) through the same `set()` pattern as the presets; `role="group"` + `aria-label="گروه‌بندی روند"` + `aria-pressed`; reset already drops `group` because it replaces the whole param set. Backend validation is not duplicated, and reports that do not enable `trendGroup` are unaffected.
- **Trend bucket boundaries + zero-fill (the real bug)** (`src/lib/profitability.ts`) — every bucket previously received the **overall report window** as its `from`/`to`. New pure exported helpers: `TrendGroup`, `startOfTrendGroup`, `nextTrendGroupStart`, `isoWeek`, `trendGroupLabel`, `computeTrendBuckets(windowFrom, windowTo, group)` → per-bucket `{ label, from, to }`, and `assembleTrendBuckets(buckets, salesByGroup, expenseByGroup)` (extracted so the mapping is unit-testable with no DB). `computeTrend` now generates **every** bucket covering the requested window up-front (inclusive per-bucket days, contiguous, non-overlapping), then maps the aggregation onto them → **empty periods are zero-filled** and **expense-only periods survive**, so expenses can no longer silently disappear from net-profit trend analysis. Labels are generated to match Mongo's `$dateToString` keys **exactly** (`%Y-%m-%d`, `%G-W%V` via a real ISO-8601 week-year implementation — the `2027-01-01 → 2026-W53` transition is covered by a unit test — and `%Y-%m`). Degenerate windows are guarded and the series is capped at 3000 buckets (a defensive payload bound; the report UI's own date-range constraints keep normal ranges far below it). `expenses` was added to the `TrendBucket` payload (it was already computed internally, only not exposed — needed by the trend sheet and the chart's accessible text; **no new accounting metric**). The date convention is documented in-code: the user/API date vocabulary stays **inclusive UTC calendar days** while the query window remains the half-open instant range the report subsystem already uses — no new timezone system.
- **Accessibility pass** (`src/components/reports/profitability-view.tsx`, no redesign) — every sortable header is now `<th scope="col" aria-sort="ascending|descending|none">` wrapping a real keyboard-operable `<button>` (direction also conveyed by `sr-only` text; the arrow glyph is `aria-hidden`), all static `<th>`s carry `scope="col"`, decorative `ChangeIcon`/trend arrows are `aria-hidden`, and the waterfall/trend/KPI/health surfaces expose textual alternatives instead of relying on bare `title=`. No charting or accessibility dependency was added.
- **Accounting invariants untouched** — COGS remains `OrderItem.fifoUnitCost → historical OrderItem.supplierPrice snapshot → 0` (the identical expression in both the aggregate and the trend pipeline); the **current** `Product.supplierPrice` is never used for historical profitability; **no `category` snapshot was added to `OrderItemSchema`**; no migration/backfill; no P&L/FIFO/inventory/checkout/payment/refund code or `report-matches.ts` changed.
- **Explicitly deferred** — historical category profitability, which requires a `category` snapshot on the embedded `OrderItemSchema` plus a backfill decision (a migration-class change). `categories` is intentionally `[]` (the UI/export keep an honest empty state rather than a fabricated figure).

### Tests & verification
- **`tests/unit/profitability.test.ts`** — bucket boundaries for day/week/month (each bucket exposes its OWN `from`/`to`, never the window — the original bug), contiguity + non-overlap, ISO week-year transitions incl. `2027-01-01 → 2026-W53`, zero-fill for empty daily/weekly/monthly buckets (all financial values 0), an expense-only period (`netSales = cogs = grossProfit = 0`, `expenses > 0`, `netProfit = −expenses`), and all pre-existing foundation cases unchanged.
- **`tests/unit/report-utils.test.ts`** — `group` parsing: `day` / `week` / `month`, absent → `undefined`, invalid → the existing parser behaviour (400 at the API layer).
- **`tests/unit/profitability-excel.test.ts` (new, offline)** — builds the **real** workbook with `buildReportWorkbook()` and re-parses it with exceljs (no server, no DB): the five expected Persian sheet names, the generic P&L summary sheet **absent**, the product sheet present with its column contract, KPI/waterfall values genuinely **numeric** (not strings) with money/percent formats, and the trend sheet carrying group/from/to plus the money columns. This regression-guards the 500 that no test previously covered.
- **`scripts/verify-reports.js` (extended, real-API)** — profitability JSON shape (`kpis`/`waterfall`/`products`/`categories`/`expenses`/`diagnostics`/`health`/`trend`), accounting reconciliation (`netSales − cogs = grossProfit`, `grossProfit − operatingExpenses = netProfit`), trend `group=day|week|month` honoured with correct non-overlapping contiguous boundaries covering the window, **Σ trend totals = KPI totals** (this is the assertion that guards the generated-label ≡ Mongo `$dateToString` equivalence), invalid `group` → 400, and the Excel export actually parsed (HTTP success + valid xlsx + the five sheets + product net-sales reconciliation).
- **Gates actually run:** `npx tsc --noEmit` **0 errors** · `npm run check` **exit 0** (0 errors, 4 pre-existing warnings) · `npx vitest run` **623/623 PASS** · targeted profitability + report-utils + excel **68/68** · `node --check scripts/verify-reports.js` OK · `git diff --check` clean.
- **BLOCKED — and therefore NOT claimed as passing:** `scripts/verify-reports.js` over HTTP, Playwright, and the full 49-suite regression. **MongoDB was unreachable from this machine for the entire session** (the app itself fails with `MongooseServerSelectionError`; `GET /api/products?limit=1` times out at 25s; TCP connects but the handshake is refused host-side — a connectivity issue, not a code issue). These three layers must be re-run once the database is reachable; the API/export and regression evidence is required before Session 88 can be called fully verified.

## Session 87 (August 2026) — Profitability Report Foundation  ⚠️ recorded retroactively (documentation gap)

> Recorded in Session 88 from commit `c75cba5` (`feat(reports): add profitability report foundation`, 22 Aug 2026). That commit carries no session heading and no CHANGELOG/PROJECT_STATE entry was written at the time; Sessions 85 and 86 were likewise left undocumented (see below). The work itself is committed and is covered by the Session 88 gates.

- **`src/lib/profitability.ts` (new)** — server-side profitability aggregation: line-level `$unwind` over order items with coupon discounts allocated proportionally by line, 10 KPIs with previous-period comparison + `changePercent`, an 8-step waterfall, product rows, expense analysis (non-void only), 8 rule-based diagnostics, 5 financial-health indicators and trend buckets. COGS = `$ifNull: [items.fifoUnitCost, items.supplierPrice]` → `0` — historical `fifoUnitCost` first, the immutable `supplierPrice` snapshot as fallback, and **never** the current `Product.supplierPrice`.
- **`src/components/reports/profitability-view.tsx` (new)** — executive KPI grid, waterfall bars, sortable product table, category table, expense table, diagnostics, health indicators, trend chart and a 3-tab ranking section (products/categories/ranking).
- **Wiring** — `REPORT_SLUGS` + `getReportData` case (lazy-imported), `REPORT_TITLES.profitability = "سودآوری"`, `REPORT_META.profitability`, export dispatch; every shared-file change additive, so the existing reports (incl. P&L) were unaffected.
- **Gaps at that commit — all closed or explicitly deferred in Session 88:** profitability export 500 (fixed), P&L metadata reused by the profitability page (fixed), no trend-group UI (added), every trend bucket carrying the overall window as its `from`/`to` (fixed), empty category profitability (deferred — needs an `OrderItem` category snapshot), no integration coverage (offline XLSX + real-API verification added).

## Session 86 (August 2026) — Storefront dark-glass consistency + prerender fix  ⚠️ recorded retroactively (documentation gap)

> Recorded in Session 88 from commit `2fa0ba1`. No CHANGELOG entry was written at the time.

- Route-aware dark toast theme via `usePathname` in Sonner (storefront = dark glass; admin/supplier/auth = light); status-chip and price contrast fixes across cart/checkout/coupons/orders/suppliers; product-description styling refinements; storefront header styling adjustment.
- **Fixed pre-existing build blockers:** `useSearchParams()` wrapped in `Suspense` boundaries on `/support`, `/admin/reports` and `/admin/reports/[report]` (required by Next.js static prerendering) — the production build passes cleanly afterwards (it was previously blocked by these prerender errors).
- Recorded result at the time: **49/49 regression suites PASS**.

## Session 85 (August 2026) — Storefront Dark-Glass Redesign (Phases A–C)  ⚠️ recorded retroactively (documentation gap)

> Recorded in Session 88 from commits `972bc3f` (Phase A) / `9f1ff34` (Phase B) / `965eb2c` (Phase C). No CHANGELOG entry was written at the time.

- **Phase A — scoped foundation:** the productcarousel/productPage visual language applied to the storefront **only**, via a scoped `.storefront` token override (homepage + the `(storefront)` route group); admin, supplier and auth keep the light theme. Near-black surfaces + a blue accent system chosen so the single `--primary` serves both text links (~5.4:1) and buttons with navy foreground (~4.9:1) — the axe gate (homepage/catalog/product detail) stays fully clean. Foundation utilities (dot-grid, glass-card, glass-strong, blue-grad) + an app-wide `prefers-reduced-motion` gate; header restyled to the glass-strong reference bar (blue logo tile, blue active-nav border, glass search pill); translucent footer border. Zero ecommerce/logic changes.
- **Phase B — glass carousels + cards:** `ProductRail` became a true horizontal scroll-snap carousel on ALL breakpoints (desktop flanking glass circular nav buttons with RTL-correct scroll mapping — the reference demo's own mapping is inverted and was not copied; keyboard-scrollable track; mobile keeps the touch/snap behaviour); `ProductCard` restyled to the glass surface with every behaviour contract preserved (effective pricing, countdown/`onCountdownExpire`, stretched link, customer-gated wishlist, supplier link, image allowlist/fallback, disabled/out-of-stock, E2E badge/button text); `SectionHeader` oversized titles + «مشاهده همه»; home surfaces (hero/banner/categories/brands/gifts/trust/skeletons) polished. No API/model/checkout/catalog/product-detail changes; no new dependencies.
- **Phase C — product detail redesign:** dark-glass product detail page + catalog (glass stage gallery with radial glow and glass thumbnails, glass price card, blue-gradient AA-safe CTA, honest stock chips with no fabricated capacity bar, glass perk tiles, real-data tabs توضیحات/مشخصات فنی/دیدگاه‌ها, a mobile-only sticky buy bar reusing the exact add-to-cart handler, glass restyles for the variant selector and reviews). Fixes found while in scope: **390px horizontal overflow** (grid columns lacked `min-w-0`), a **double «تومان»** in the price text, and a subtitle that duplicated description text into a second node (breaking single-match E2E text contracts) was removed; `.first()` disambiguation where the mobile sticky bar legitimately duplicates the CTA/price text. **No related-products rail** — no real related-product data source exists in the app (the `related*` refs are notification/refund order refs), so the section was omitted rather than fabricated.
- Recorded results at the time: tsc 0 · check 0 errors (4 pre-existing warnings) · Vitest 580/580 · targeted E2E 55/55 · browser QA 63/63 across 1280/1920/1024/768/390 (no overflow, sticky bar per breakpoint, tabs/lightbox/fallback/variants, zero console/hydration/duplicate-key errors) · full Playwright 122/123 (only the pre-existing Parspack rich-description upload flake, reproduced at clean HEAD) · full regression 48/49 (`verify-payment-retry` sandbox rate-limit flake, passes 16/16 standalone — the documented Session 84 pattern).

## Session 84 (August 2026) — Forgot Password / Password Recovery (بازیابی رمز عبور)

### The approved scope (auth/security feature on the Session 83 UI — zero changes to the existing auth core)
- **Dedicated purpose-scoped OTP flow** — `purpose: "password_reset"` added to the EXISTING `OtpCode` infrastructure (enum + `resetTokenHash`/`resetTokenExpiresAt` fields — additive; login/register OTP contracts untouched). The request/verify routes reuse the same validation-before-rate-limit convention and the same shared per-phone OTP/SMS budget (`otp_request:<phone>` 5/15min, per-IP 15/15min, 60s resend cooldown) — ONE SMS-bombing budget across login/register/recovery.
- **Closed anti-enumeration cooldown oracle** — for `password_reset`, an UNKNOWN phone now receives the same `200 { sent: true }` AND a dummy `OtpCode` row (random code hash never delivered, `devPlaintextCode: null`), so the 60s resend cooldown behaves IDENTICALLY for known and unknown numbers (no 200-vs-429 existence oracle; the `login` purpose keeps its documented legacy trade-off). No SMS is ever sent to an unknown phone.
- **OTP verify mints a reset token — never a session** — `password_reset` verify issues a **256-bit `resetToken`** (SHA-256 stored as `resetTokenHash`, 2-min TTL aligned with the row) instead of a `loginToken`. A recovery can NEVER create a session; the flow never calls `signIn()`. Reset tokens are cryptographically random, short-lived, single-use (atomically consumed in complete), and held only in React memory.
- **`POST /api/auth/password-reset/complete`** — token-gated set-new-password (bcrypt 10, SAME 6–100 policy as register/change-password via the existing `registerSchema` password rule), **`tokenVersion` bump + cache invalidation** (every previously-issued session dies, exactly like change-password), and **outstanding-OTP invalidation**: every `OtpCode` row for the phone (login/register/reset codes AND issued-but-unspent login/reset tokens) is marked consumed so a pending pre-reset `loginToken` can never authenticate. Public endpoint (possession of the token is the authorization); uniform «درخواست نامعتبر یا منقضی شده است» for unknown token/account; rate-limited 5/phone/15min. Passwordless (OTP-only) accounts can establish their FIRST password through this flow.
- **UI (Session 83 visual language, `(auth)` route group)** — `/forgot-password` four-step flow: phone (generic anti-enumeration note) → OTP (resend countdown) → new password + confirm (zod `resetPasswordSchema`) → success + «ورود به حساب کاربری». Entry point «رمز عبور را فراموش کردهاید؟» on the login page only (password tab). Holographic split-screen, RTL, Vazirmatn, `resetToken` in React memory only.
- **Shared rate-limiter race fixed (real bug found during verification)** — `src/lib/rate-limiter.ts` used a non-atomic find-then-create for a key's first request: two CONCURRENT first requests both saw no doc and both created → E11000 → the loser 500'd. The concurrent-complete test caught exactly this. The create is now E11000-guarded (the loser counts as its own fresh window; every SUBSEQUENT request increments normally) — the anti-abuse budget is preserved and the concurrent loser returns its clean 400.

### Tests & verification
- **`scripts/verify-password-reset.js` — 22/22 real-API** (self-cleaning, PREFIX-scoped): mock seam, known/unknown same 200 shape + nothing stored for unknown, dummy-row cooldown 429 with Retry-After for BOTH known and unknown, wrong code + attempts, cross-purpose isolation both ways (login code cannot verify a reset, reset code cannot login), resetToken-only response (no loginToken, hashed at rest, code consumed), verify creates no session, full reset (old password dies, new works, tokenVersion bumped), pre-reset session revoked (protected API 401), token replay 400, CONCURRENT completes exactly one 200 (loser 400), IDOR cross-phone 400, passwordless first-password, expired OTP/token 400, wrong-code lockout, short password 400, rate limits (verify + complete 429), outstanding login token invalidated by the reset, login/register OTP unchanged.
- **`tests/unit/password-reset.test.ts`** — reset-token helpers (generate/hash/normalize), purpose gating, complete-route validation semantics; Vitest **580/580** (570 + 10).
- **Journey 18 E2E** — `tests/e2e/password-reset.spec.ts` (desktop: full recovery loop + wrong-OTP rejection) + `tests/e2e/password-reset-mobile.spec.ts` (mobile RTL smoke — **anonymous** unknown-phone path so it never consumes the shared register budget). **Suite-level register-budget discipline**: the desktop journey sweeps ONLY the `rl:register:` keys before its single shared registration (a full run already sits at the 5/15min register budget via global-setup + otp-login + session-security + supplier-application; no E2E spec asserts a register 429).
- **playwright.config.ts** — mobile `testMatch` gains `password-reset-mobile`; accessibility spec scans `/forgot-password` (axe AA, zero serious/critical).
- **Regression runner** — `verify-password-reset` added → **49 suites** (regression.yml renamed); the pre-suite rate-limit sweep already covers `otp_request|otp_request_ip|otp_verify`; the suite self-sweeps `password_reset_complete` + `register` fixture keys and crashed-run `PR pr84_` residue.
- **Verified:** tsc 0 · `npm run check` 0 errors (4 pre-existing warnings) · Vitest **580/580** · `verify-password-reset` **22/22** · auth E2E subset **36/36** (customer-login, otp-login, session-security, accessibility/axe, logout, password-reset ×2, password-reset-mobile ×2) · full Playwright **122/123** (the single failure is the **pre-existing external-Parspack rich-description upload flake** — reproduced identically at clean HEAD via `git stash`; nothing in Session 84 scope) · full regression **48/49** — the single failure is the **pre-existing Zarinpal-sandbox flake in `verify-payment-retry`** (the full run exhausts the real sandbox's authority rate limit; the suite passes **16/16 standalone** on a clean window, matching the documented sandbox behavior; nothing in Session 84 scope — auth-only changes).

## Session 83 (August 2026) — Login/Register UI Redesign (marloo-login)

### The approved scope (visual redesign only — zero authentication-logic changes)
- **New `(auth)` route group** (`src/app/(auth)/login` + `register`): the two auth pages moved out of the `(storefront)` layout (header/footer) into a minimal full-viewport layout so the split-screen design owns the viewport. **Public URLs unchanged** (`/login`, `/register`) — middleware, NextAuth `pages.signIn`, storefront header links and every `router.push("/login")` caller are untouched.
- **Design reproduced from the marloo-login reference** (adapted Persian/RTL, Vazirmatn, no new dependencies): a **split screen** — dark holographic scene (`56%`, hidden below `lg`) with aurora glows, tech grid, deterministic starfield, light-trail network, holographic orb + orbit rings, floating product icons, glass HUD chips (Persian), scanline + vignette; and a **white panel** (`44%`) with ambient cyan/purple glows, eyebrow + gradient rule, hero heading (with the E2E-asserted «ورود به حساب کاربری» / «ساخت حساب کاربری» titles), rounded-2xl icon inputs with cyan focus rings, password eye toggles, neon shimmer CTA with loading state, staggered entrance (translateY only — see axe note), and a security footer.
- **Only real features rendered**: the reference's Google/Apple buttons, remember-me and forgot-password are NOT shown (the app has no such features); the real method tabs («ورود با رمز عبور» / «ورود با کد یکبارمصرف») remain the alternative-login mechanism. `OtpPanel` + `OtpCodeInput` restyled to the same language; every label, button, error text, endpoint call and the whole OTP state machine is byte-identical.
- **Accessibility (axe AA contract kept):** the entrance animation animates translateY only (an opacity fade blended colors mid-flight and produced false color-contrast violations); the panel heading accent uses an accessible cyan-700 glow (`.neon-accent`) instead of bright `#00e5ff` on white; the CTA gradient is deepened (cyan-700 → purple-600) so white text holds ≥4.5:1; the scene is `aria-hidden` (keeps the vivid neon).
- **Two real bugs found during verification and fixed:** (1) wrapping the shadcn `FormControl` (Slot) with a `div` duplicated the input id — `FormControl` injects `id={formItemId}` onto its direct child, so the icon wrapper now sits OUTSIDE `FormControl` (the duplicate id broke `<label for>` association and `getByLabel`); (2) the password eye toggle's `aria-label` («نمایش رمز عبور») contained the password label as a substring, so Playwright's substring `getByLabel("رمز عبور")` matched two elements (strict-mode violation) — the toggle now uses «گذرواژه».
- **Verified:** tsc 0 · `npm run check` 0 errors (4 pre-existing warnings) · Vitest **570/570** · auth E2E subset **22/22** (customer-login, otp-login, session-security, accessibility/axe, logout) · browser QA on desktop + mobile (`390px`): zero console/hydration/duplicate-key errors, no horizontal overflow, RTL intact, scene hidden on mobile · **full Playwright 118/118** (both mocks) · `git diff --check` clean. One commit.

## Session 82 — Phase C Hardening (August 2026) — FIFO rollback + payment-retry integrity

### The approved scope (stabilization of the production-readiness audit findings — no new features)
- **HIGH-1 — checkout SupplierOrder-failure rollback now restores FIFO layers** (`src/app/api/checkout/route.ts`): the SO-creation catch previously restored only `Product.stock` via a manual `restoreStock` mapping that dropped the FIFO consumption snapshot — a purchased sale that consumed layers and then hit an SO write failure left `stock` restored but `costLayers` permanently consumed (desync, understated inventory value, silent ledger). The catch now passes `r.fifoUnitCost` into `restoreStock` (stock + layers always move together, exactly like the `restoreReserved` paths) and deletes any SupplierOrders already created for the rolled-back order (no orphaned payout obligations pointing at a deleted order). Consignment behavior unchanged; no `supplierPrice` fallback.
- **HIGH-2 — payment-retry is now FIFO-integrated** (`src/app/api/payment/retry/route.ts`): (A) rollback — a failed purchased re-reservation restores the EXACT layers consumed by that retry (`fifoUnitCost` carried through `rollbackReserved` → `restoreStock`); (B) success — the ACTUAL re-consumed FIFO cost is persisted onto the affected `OrderItem.fifoUnitCost` (never the stale snapshot) and a compensating `sale` movement is recorded per retry cycle with a per-cycle `retryToken`-suffixed `sourceRef` (unique per attempt, so a repeated request can never double-write a movement; ledger reconciles sale −N / restock +N / retry sale −N = net held stock). Movement writes are best-effort/fail-silent; the retried Zarinpal redirect is never blocked by a ledger write; idempotent retries never double-consume.
- **MEDIUM-4 — adjustment crash-recovery** (`src/lib/inventory-adjustments.ts` + `InventoryMovement.completedAt`): the movement claim row is the exactly-once claim, and `completedAt` is stamped together with the final cost figures ONLY after the atomic stock/layer apply succeeds. A retry hitting a **pending** claim (crash between insert and apply) now COMPLETES the apply instead of short-circuiting idempotent (the pre-fix window silently lost adjustments). Layer effects are recomputed from current product state on every apply attempt (concurrent-op safe); hard failures remove only the caller's own claim.
- **MEDIUM-5 — new-variant stock bypass closed** (`assertNoDirectStockChange`): a NEW variant added post-cutover may no longer introduce `stock > 0` without an audited adjustment/cost layer (400 «از طریق تعدیل موجودی»); `stock = 0` variants and non-stock edits remain allowed; pre-cutover behavior unchanged.
- **MEDIUM-7 — purchase over-payment rejected** (`/api/admin/purchases/[id]/pay`): `amount + amountPaid > total` → 400 with a clear message instead of silently capping with `Math.min` (a silently-capped entry masked data-entry errors in the ledger). Nothing is mutated on rejection; exact/under/idempotent payment unchanged.
- **MEDIUM-1 — cutover date can never be in the future** (config + initialize routes): a future `cutoverDate` (which would misclassify sales before it as post-cutover via the `fifoUnitCost` presence rule) → 400 «تاریخ شروع حسابداری نمی‌تواند در آینده باشد».
- **MEDIUM-6 — misleading `sourcing_change` filter removed**: the movement-type UI chip (`/admin/inventory`) and API filter whitelist no longer offer `sourcing_change` (no code path produces it — the init wizard converts sourcing but writes no such movement). The label map + TS union keep the reserved value so legacy rows still render/type-check.

### Tests & verification
- `verify-fifo` **16/16** — new TEST 16 forces the SO-creation failure deterministically (a **partial unique index** on `order` keyed on the shared consignment item name — a plain unique `order` index can never be built in this app because multi-supplier orders legitimately share an order id across SOs) and proves stock, layer quantity AND layer value are restored exactly, the order + every SO are cleaned up, and no `sale` movement leaks.
- `verify-payment-retry` **16/16 ×2** (sandbox-rate-limit-free runs) — new TEST 14 (purchased retry success: `fifoUnitCost` refreshed from a stale 999 to the actual consumed 1000, stock/layers exactly 8, single `sale-…-retry-` movement), TEST 15 (partial re-reservation failure → stock AND layers restored exactly, `payment.status` back to failed, zero movements), TEST 16 (concurrent purchased retries → one 200/one 409, single reservation, single movement).
- `verify-inventory` **16/16** — TEST 14 extended with the MEDIUM-5 new-variant guard (stock>0 → 400, stock=0 → 200; real Attribute fixture so the route's unrelated variant-attributes validation doesn't mask the guard) + new TEST 14b crash-recovery (pending claim row completed on retry at the confirmed cost, single movement row, repeat key → idempotent, no double apply).
- `verify-purchases` **17/17** — new over-payment test: amount beyond remaining → 400 + nothing mutated, exact remaining payment still settles; placed AFTER the purchases-report outstanding assertion (which expects 470,000 outstanding on the same fixture).
- `verify-accounting` all pass — new TEST 3b: future cutover → 400; past/today still accepted.
- Gates: `tsc` 0 · `npm run check` exit 0 (4 pre-existing warnings) · Vitest **570/570** · all accounting suites green (verify-fifo 16/16 · verify-payment-retry 16/16 · verify-inventory 16/16 · verify-purchases 17/17 · verify-expenses 10/10 · verify-accounting-excel 10/10 · verify-reports) · `git diff --check` clean. Full Playwright + full regression re-run in the regression environment (see final report).

## Session 82 Phase F (August 2026) — Accounting Excel V2 Exports

### The approved scope (Session 82 Phase F — additive, backward-compatible Excel V2 on top of Phases A–E)
- **`accounting` report slug** (`REPORT_SLUGS`/`REPORT_TITLES`/`REPORT_NAV`/`REPORT_META` + report-filters + `/api/admin/reports/accounting` JSON + `/export` xlsx) — the 17-sheet accountant workbook: خلاصه · فروش · سفارش‌ها · اقلام فروش · پرداخت‌ها · مرجوعی‌ها · مشتریان · کوپن‌ها · خریدها · اقلام خرید · هزینه‌ها · موجودی · گردش موجودی · لایه‌های FIFO · بهای تمام‌شده · سود و زیان · خلاصه حسابداری. Every sheet reuses the EXISTING report services (Session 81 dashboard/detail sheets byte-unchanged; new sheets are additive).
- **New server-side data service `src/lib/accounting-v2.ts`** — `getOrderItemsReport` (per-line historical snapshot: unitPrice/originalPrice/productDiscount/couponAllocation/netSales + **FIFO COGS** — `fifoUnitCost` when present, else the immutable `supplierPrice` snapshot — + grossProfit/grossMargin), `getPurchaseItemsReport` (ordered/received/remaining, unit cost from the PurchaseItem — never current supplierPrice), `getInventoryMovementsReport` (append-only ledger with Persian type labels + createdBy/variant labels; null-product-safe after deleted products), `getCostLayersReport` (active FIFO layers only, remainingValue = remaining × unitCost, Σ = inventory value), `getCogsReport` (per-line COGS with `cogsSource` fifo|snapshot + Persian label + `sourceRef sale-<order>-<product>[-<variant>]`), `getAccountingSummary` (KPIs + **valuation basis label** — «ارزش موجودی بر اساس لایه‌های FIFO» vs the pre-cutover approximation — + the historical snapshot note).
- **`src/lib/reports-excel.ts` V2** — `buildAccountingWorkbook` + 7 new column sets (اقلام فروش، اقلام خرید، هزینه‌ها، گردش موجودی، لایه‌های FIFO، بهای تمام‌شده) + `writeAccountingSummarySheet`: label/value KPIs with money/percent/count formats, `null` rendered as **«در دسترس نیست»** (never a fake zero), and the accounting-notes block (FIFO method, cutover note, «هیچ مبلغی تخمین زده نمی‌شود»). Existing `buildDashboardWorkbook` untouched.
- **`src/lib/report-matches.ts`** — the shared order/line match builders + window/escape helpers extracted from `reports.ts` (circular-import fix: both `reports.ts` and `accounting-v2.ts` import from it). Behavior byte-identical.
- **Generic report page renders `accounting`** via the standard envelope (rows = order-items table, SummaryCards + totals + export button) — no parallel catalog/UI implementation.
- **`buildStatementRows`/P&L unchanged** — the accounting workbook reuses the P&L sheet with `operatingExpenses` + `netProfit = grossProfit − operatingExpenses`; Σ COGS rows = P&L COGS; Σ active FIFO layer values = inventory value (both asserted in the real-API suite by parsing the generated XLSX).

### Tests & verification
- **Unit `tests/unit/accounting-excel.test.ts` — 6 hermetic** (cogsSource fifo/snapshot classification, MOVEMENT_LABELS/LABELS coverage, layer remainingValue math, summary netProfit = grossProfit − operatingExpenses, P&L row keys/labels) — Vitest **570/570** (564 + 6).
- **`scripts/verify-accounting-excel.js` — 10/10 real-API** (auth 401/403 ×report+export; purchased sale → `fifoUnitCost` snapshot + **NO SupplierOrder**; consignment sale → SupplierOrder + no FIFO; receipt movement `receipt-<purchase>-<key>-<item>` sourceRef; expense create/pay/void with audited void; **XLSX parsed reconciliation**: sheet set + Persian headers, Σ COGS rows = P&L COGS, Net Sales − COGS = Gross Profit, Gross Profit − Operating Expenses = Net Profit, Σ active FIFO layer values = inventory value, purchase/expense totals, «در دسترس نیست» on null, rate-limit 429 on export). Self-cleaning (login keys cleared in the sweep; per-run PREFIX cleanup; SupplierOrder assertions by unique order id — immune to leftover cross-run docs). Hermetic ×4.
- **Journey 23 `tests/e2e/admin-accounting-excel.spec.ts` — 2/2** (purchase → receive → FIFO layer → real storefront sale → COGS → expense → P&L → real Excel download parsed for the 17 Persian sheets + values; `?report=accounting` page render). Cross-spec pollution fix: scoped `afterAll` purge of **own PREFIX'd** rows (products/purchases/movements/expenses) — never touches shared users/suppliers — so `admin-expenses`' exact monthly total stays isolated (the earlier full-suite failure). Hermetic ×2.
- **`scripts/run-regression.js` + `regression.yml`** — `verify-accounting-excel` added after `verify-expenses` → **48 suites**.

### Verified
- tsc 0 · `npm run check` exit 0 (4 pre-existing warnings — none in new files) · Vitest **570/570** · `verify-accounting-excel` **10/10** (hermetic ×4) · Phases A–E suites green (`verify-accounting` 14/14 · `verify-purchases` 16/16 · `verify-fifo` 15/15 · `verify-inventory` 15/15 · `verify-expenses` 10/10 · `verify-reports` 21/21) · targeted E2E **2/2** (Journey 23, hermetic ×2; isolated with `admin-expenses` + `admin-inventory` 9/9 green) · **full Playwright 117/118** (chromium 105 + mobile 12; the single failure is the **pre-existing `product-rich-description` external-Parspack-host flake** — the Playwright browser cannot reach the bucket (20s timeout) while curl returns 200 in 0.15s; API round-trip + stored node + URL all asserted green before the lazy-load visibility line; touches nothing in Phase F scope) · full regression **48/48 PASS** (48 suites, `SMS_MOCK=1` only). Accounting-integrity review: all 9 traceability questions answer YES (COGS → snapshot/layer source; inventory → active FIFO layers; net profit → P&L; void expenses excluded from totals but retained; pre-cutover values preserved; unavailable marked «در دسترس نیست»; sales→COGS→profit, purchases→receipts→layers→inventory, expenses→P&L→net profit all traceable).

## Session 82 Phase E (August 2026) — Expense Ledger + Net Profit + P&L V2

### The approved scope (Session 82 Phase E — build on Phases A–D, no changes to FIFO/procurement/movement/checkout)
- **`Expense` model** (`src/models/Expense.js`) — category (the approved 10-category enum: shipping/packaging/advertising/gateway_fees/rent/utilities/salaries/software/maintenance/other), description 2–500, amount (integer ≥ 0 Toman), expenseDate, paymentMethod (cash/bank/card/online/other), reference ≤200, payee ≤200, notes ≤1000, status (`paid | pending | void`), createdBy/updatedBy refs, voidedAt/voidedBy/voidReason. **Audited void:** voidReason REQUIRED (2–500, validated before the rate limiter), the row is NEVER hard-deleted (stays visible with void metadata for the audit trail), voids are final (no un-void — corrections are new rows), voided pay/update → 400. Nothing is ever fabricated — a gateway fee is only counted when recorded manually as `gateway_fees` (the approved decision: no estimation).
- **5 additive admin APIs** (all `requireRoleOrError(["admin"])`, malformed ObjectId → 400, unknown → 404, validation BEFORE the `EXPENSE_WRITE_LIMIT` 60/actor/15min rate limiter): `GET/POST /api/admin/expenses` (list with status/category/date-range/q filters + pagination; create defaults to `pending`), `GET/PATCH /api/admin/expenses/[id]` (update while not void — voided → 400), `POST /api/admin/expenses/[id]/pay` (pending → paid, **idempotent**, voided → 400), `POST /api/admin/expenses/[id]/void` (audited void).
- **Expenses report** (`/api/admin/reports/expenses`, kind `expense` in `parseReportFilters`) — rows: date/category/description/amount/method/reference/payee/status/createdBy/void audit; **totals count ONLY non-void expenses** (voided rows stay visible for audit but never affect money); filters: preset/from/to + expense status + expense category enum + q. Registered in `REPORT_SLUGS`/`REPORT_TITLES`/`REPORT_NAV`/`REPORT_META` + report-filters (expense-labeled status + category selects) + expense-labeled summary cards.
- **P&L V2** — `getProfitLossReport` now aggregates **operating expenses = Σ non-void expense amounts in the window** for the current AND previous windows; `buildStatementRows` gains an `operatingExpenses` line (−Σ) and **`netProfit` is now a real value: Net Profit = Gross Profit − Operating Expenses** (no longer flagged unavailable — the expense ledger is authoritative; a real zero is not a fake zero); `change.netProfit` added. PnlView footnote + netProfit change chip updated.
- **Excel** — the expenses export (`/api/admin/reports/expenses/export`) writes an expense-labeled Summary sheet (count/operating total/paid/pending/voided, with the «never estimated / voided excluded» note) + an `هزینه‌ها` detail sheet (styled, formatted); the dashboard workbook's Summary note no longer claims net profit is unavailable.
- **Admin UI** — sidebar «هزینه‌ها» (ReceiptText icon) → `/admin/expenses` (status tabs, category filter, search, pagination, inline create form) + `/admin/expenses/[id]` (detail, edit-while-not-void, mark-paid, audited void modal with required reason + audit block for voided rows). `aria-label`s on all form controls.
- **Mobile-drawer fix (needed by the new sidebar item):** the admin mobile drawer scrolled the WHOLE aside, so the 22-item nav pushed «خروج» below the fold (y=1093 vs the 851px Pixel-5 viewport) and the logout E2E force-click landed on «کاربران». The drawer now pins the store-link/logout footer while only the `<nav>` scrolls (`min-h-0 overflow-y-auto`) — matching the desktop column; verified the logout button is visible without any scroll and the plain click navigates to `/`.
- **Pre-existing test updates (required by the approved Phase E semantics):** `verify-reports.js` TEST 7 and `tests/e2e/admin-reports.spec.ts` asserted «net profit must be unavailable» — updated to assert netProfit = grossProfit − operatingExpenses (0 in their fixtures → netProfit = grossProfit, available). No assertion was weakened; the behavior they encoded changed per the approved design.

### Tests & verification
- **Unit `tests/unit/expenses.test.ts` — 16 hermetic** (parseReportFilters expense kind incl. invalid status/category + product-kind unchanged, validateExpenseBody validation matrix, P&L `buildStatementRows` netProfit math + statement-line order + zero-net safety) — Vitest **564/564** (548 + 16).
- **`scripts/verify-expenses.js` — 10/10 real-API** (401/403 matrix ×6 endpoints, create+validation, detail/list/search/malformed→400/unknown→404, update-while-pending, voided-update 400, pay pending→paid idempotent + voided-pay 400, audited void (reason required, row never deleted, detail exposes void audit), expenses report totals EXCLUDE voided + status filter, P&L netProfit = grossProfit − operatingExpenses, report slug + invalid filters 400, rate-limit 429). Self-cleaning, hermetic ×2.
- **Journey 22 `tests/e2e/admin-expenses.spec.ts` — 2/2** (UI create → list → detail mark-paid → second expense audited-voided with required-reason validation; expense report excludes the voided row from totals while showing it for audit; P&L operating-expenses/net-profit rows present and netProfit = grossProfit − expenses). Hermetic ×2.
- **`scripts/run-regression.js` + `regression.yml`** — `verify-expenses` added after `verify-inventory` → **47 suites**.

### Verified
- tsc 0 · `npm run check` exit 0 (4 pre-existing warnings) · Vitest **564/564** · `verify-expenses` **10/10** (hermetic ×2) · Phases A–D suites still green (`verify-accounting` 14/14 · `verify-purchases` 16/16 · `verify-fifo` 15/15 · `verify-inventory` 15/15) · `verify-reports` **21/21** (after the Phase E assertion update) · targeted E2E **2/2** (Journey 22, hermetic ×2) · **full Playwright 116/116 PASS** (chromium 104 incl. Journeys 19/20/21/22 + mobile 12 — the mobile logout flake from the new nav item is FIXED by the drawer footer-pin) · full regression **47/47 PASS** (47 suites, `SMS_MOCK=1` only; first run had a single `verify-rich-description` failure from a **transient MongoDB connectivity drop** — ETIMEDOUT to the Chabokan DB host `91.236.168.95:2255`, the documented CHANGELOG pattern — it passed 23/23 standalone and the re-run was all-green). Phases F (Excel V2) NOT started.

## Session 82 Phase D (August 2026) — Inventory Adjustments + Movement Ledger

### The approved scope (Session 82 Phase D — complete the inventory audit layer around the FIFO system)
- **Shared adjustment service `src/lib/inventory-adjustments.ts`** — the single audited path for stock changes outside the existing flows (purchases/receiving, checkout, refunds, cancels). `applyInventoryAdjustment()` validates (mandatory reason 2–200, integer non-negative unitCost, malformed ids → 400, insufficient stock → 409 with NO partial mutation), performs a **single-document atomic update** (stockVersion optimistic lock + 5-attempt re-read retry — the established pattern), creates an **append-only `InventoryMovement` of type `adjustment`** (product/variantId/quantity/unitCost/totalCost/sourceRef/description/createdBy) and is **exactly-once idempotent via the movement's unique partial index on `sourceRef`** (+ an `exists()` pre-check as belt-and-suspenders — a repeated same-key request never double-adjusts). Positive adjustments create a NEW FIFO cost layer at the explicitly-supplied cost (never silently the current supplierPrice); negative adjustments consume FIFO layers oldest-first with an auditable cost effect (fail-safe when layers can't represent the reduction).
- **Post-cutover enforcement** — once accounting is initialized (`accountingconfigs.inventoryInitialized`), **direct stock writes are rejected**: admin `PUT /api/admin/products` with a changed `stock`/variant stock → 400 «…از طریق تعدیل موجودی…» (non-stock edits untouched), and the supplier stock quick-edit → 400. Pre-cutover behavior is unchanged. All stock mutation paths are now audited: checkout/reserve + every restoration (payment failure/cancel/refund), purchase receiving, and adjustments.
- **3 additive admin APIs** — `POST /api/admin/inventory/adjustments` (admin-only, `INVENTORY_WRITE_LIMIT` 30/actor/15min, validation-before-rate-limit), `GET /api/admin/inventory/movements` (filters: product, variant, type, sourceRef, date range, search, paginated — answers چه چیزی/چه زمانی/برای چه محصولی/چقدر/با چه بهایی/به چه دلیل/توسط چه کسی/منبع), `GET /api/admin/inventory/layers` (active FIFO layers only, value = remaining × unitCost, acquiredAt, source, ref — consumed layers stay auditable through movements). All `requireRoleOrError(["admin"])`, malformed ObjectId → 400, no customer PII.
- **`/admin/inventory` UI** (sidebar «انبار») — inventory summary (stock + FIFO layer count + value), adjustment form with live preview (current stock → requested → result + cost impact) and a **confirmation modal** before committing, movement history with type/product/date filters, and the FIFO layer viewer. «در دسترس نیست»/«نامشخص» for uncomputable values (never fake zeros).
- **`InventoryMovement` unique-index fix (real bug found + fixed):** the model's unique partial index on `sourceRef` (declared `{ $type: "string", $ne: "" }`) **never built anywhere** — MongoDB rejects `$ne` inside `partialFilterExpression`. The receive route's `exists()` pre-check masked it; the adjustment service relied solely on the index for exactly-once, so sequential duplicates slipped through. Fixed to `$gt: ""` (same semantics, supported) and the stale `sourceRef_1` index was rebuilt as unique in the dev DB. This also hardened the Phase B receive path's dedupe.
- **Reports integration** — inventory report keeps layer-based valuation for post-cutover purchased products (Σ remaining × unitCost), never reintroducing stock × supplierPrice for initialized FIFO inventory; reconciliation invariant verified (Σ movements == stock, Σ layer remaining == stock for initialized purchased products; consignment documented/tested separately).

### Tests & verification
- **`src/lib/inventory-adjustments.ts` + unit tests `tests/unit/inventory-adjustments.test.ts`** — positive/negative adjustment math, layer creation/consumption, insufficient layers, reconciliation, idempotency, variant isolation (**11 new**, Vitest **548/548**).
- **`scripts/verify-inventory.js` — 15/15 real-API** (401/403 matrix, positive/negative adjustment + exact FIFO layer semantics, insufficient stock, malformed ids, mandatory reason, exactly-once duplicate request, concurrent no-oversell, variant isolation, consignment stock-only (no layer, cost 0), movement filters, layer API, post-cutover enforcement (admin PUT + supplier stock-edit 400, non-stock edit 200), reconciliation). Self-cleaning, restores the config singleton it flips.
- **Journey 21 `tests/e2e/admin-inventory.spec.ts` — 5/5** (init → receive → FIFO layers UI → positive adjustment (new layer at confirmed cost) → negative adjustment (FIFO consumption) → movement ledger rows → post-cutover direct-edit rejection).
- **Cross-suite singleton-leak fix (root cause of the earlier 3-failure full Playwright run):** the accounting-wizard E2E journeys (`admin-inventory`, `admin-purchases`, `fifo-sale`) stamp the GLOBAL `accountingconfigs` singleton — leaving it stamped through the whole run leaked the Phase D stock-edit enforcement into later specs (product-discount's `PUT` with `stock` → 400 → its still-active discounted product then broke product-listing-seo's 2-badge assertion). Fixed per the verify-accounting.js convention: each journey now cleans the singleton in `afterAll` (`tests/e2e/helpers/db.ts` gained `clearAccountingConfig`/`stampAccountingConfig`; admin-inventory re-stamps on its beforeAll recycle path).
- **`scripts/run-regression.js` + `regression.yml`** — `verify-inventory` added after `verify-fifo` → **46 suites**.

### Verified
- tsc 0 · `npm run check` exit 0 (4 pre-existing warnings) · Vitest **548/548** (537 + 11) · `verify-inventory` **15/15** (hermetic ×2) · `verify-accounting` **14/14** + `verify-purchases` **16/16** + `verify-fifo` **15/15** (Phases A/B/C still green) · targeted E2E **5/5** (Journey 21, hermetic ×2) · previously-failing trio (payment/product-discount/product-listing-seo) **21/21 green** after the singleton-leak fix · Playwright **chromium 101/102 + mobile 12/12** (single failure = the **pre-existing Session 81 external-host rich-description flake** — reproduced independently: the browser currently cannot reach parspack.net at all (direct fetch times out) while curl gets 200; spec + storefront untouched by this session) · full regression **46/46 PASS** (46 suites, `SMS_MOCK=1` only).

## Session 82 Phase C (August 2026) — FIFO Checkout Integration, COGS & Historical Compatibility

### The approved scope (Session 82 Phase C — the most sensitive phase: replace future sales' snapshot-only COGS with actual FIFO cost-layer consumption while preserving all historical Session 81 data)
- **FIFO sale consumption folded into the shared reservation helper** (`src/lib/inventory.ts` — still the SINGLE stock authority): `reserveStock()` for `sourcing: "purchased"` products now consumes the FIFO cost layers IN THE SAME single-document atomic `findOneAndUpdate` as the stock `$inc` (stockVersion optimistic lock + re-read retry loop, max 5 attempts) — Mongo serializes per-document writes, so two simultaneous checkouts can never consume the same FIFO quantity. The consumed snapshot (`consumeFifoLayers` result + exact weighted `fifoUnitCost = consumedCost / quantity`) rides back on `__fifoConsumption`. Consignment reservations are byte-identical to pre-Phase-C (stock-only $inc). **Insufficient FIFO layers → fail-safe** (null → checkout 409, never invents cost from supplierPrice). **Variant isolation:** each variant consumes ONLY its own `variants.$[].costLayers`.
- **OrderItem.fifoUnitCost snapshot** (additive, `src/models/Order.js`): checkout stores the exact weighted FIFO unit cost for purchased-sourcing items; `supplierPrice` stays untouched (historical compatibility). Pre-cutover/consignment items have `fifoUnitCost: null`. Immutable — later purchases/receipts never alter an existing order. Types updated (`AdminOrderItem.fifoUnitCost`).
- **Supplier payout safety (the most important rule):** purchased-sourcing units are the store's OWN inventory (supplier already paid via the purchase) — checkout creates **NO SupplierOrder/amountOwed** for them (`supplierItemsMap` split by `product.sourcing`). Consignment units keep the existing SupplierOrder → amountOwed → payout flow byte-for-byte. A purchased sale can never create both FIFO COGS AND a supplier payout for the same units.
- **Checkout rollback is FIFO-aware:** `restoreReserved`/price-change/isActive rollbacks pass the snapshot `fifoUnitCost` to `restoreStock()` → the exact consumed layers are restored at their ORIGINAL unit cost (never current supplierPrice). `restoreStockWithLayers` rounds the fractional fifoUnitCost to whole toman (multi-layer sales) before `restoreLayers` — fixes a real bug where a fractional cost would throw inside `restoreLayers` and skip the restore.
- **Restoration paths all FIFO-aware via the shared `restoreOrderStock()`** (payment failure, customer cancel, admin cancel, refund): purchased items restore stock AND their exact layers + a compensating append-only movement (`cancellation_restock` / `return_restock`); consignment/pre-cutover items keep the stock-only restore. Idempotent via the existing `Order.stockRestored` atomic claim + the movements' unique sourceRef partial index. The refund route now passes `"return_restock"` (was the default cancellation label).
- **Append-only sale movements:** every purchased-sourcing sale records an `InventoryMovement` `type: "sale"` (negative quantity, unitCost = fifoUnitCost, totalCost = fifoUnitCost × qty, sourceRef `sale-<orderId>-<productId>[-<variantId>]`) — recorded only after the order + supplier orders fully commit (fail-silent, deduped by the unique sourceRef).
- **Reports V2 (`src/lib/reports.ts`, additive):** COGS now uses `$ifNull: ["$items.fifoUnitCost", "$items.supplierPrice"]` in every aggregation — purchased post-cutover sales cost from their FIFO snapshot, everything else (consignment/pre-cutover) from the immutable supplierPrice snapshot; historical report results for pre-cutover periods are unchanged. **Inventory VALUE for post-cutover purchased products comes from the FIFO layers** (Σ remaining × unitCost across Product/variant costLayers, via `layersValue`) in both `computeInventoryValue` and the inventory report rows — never current stock × current supplierPrice; unlayered (pre-cutover/consignment) inventory keeps the documented current-cost approximation. Header docs updated. Also fixed a **pre-existing Session 81 bug**: the sales report summary projected `qty: 1` (a nonexistent top-level field → `unitsSold` permanently 0); now `qty: "$items.quantity"`.
- **No changes** to coupons, Zarinpal/payment verification, order totals, the Product/Order discount models, supplier permissions, purchase receiving (Phase B), the accounting init wizard (Phase A), or the sourcing invariant. No schema migration; only the additive `OrderItem.fifoUnitCost` field.

### Tests & verification
- **`tests/unit/fifo-sale.test.ts`** — 9 hermetic tests over the pure layer helpers for the sale-integration scenarios (15-across-two-receipts exact COGS + remaining valuation, supplierPrice-change immunity, partial consumption, consume→restore roundtrip, FIFO ordering after restore, variant isolation, insufficient-layer fail-safe, weighted fifoUnitCost math, exact-boundary consumption). Vitest **537/537** (528 + 9).
- **`scripts/verify-fifo.js`** — **15/15 real-API** (self-cleaning, PREFIX'd): auth matrix (401/403), consignment sale keeps SupplierOrder payout + fifoUnitCost null + no sale movement, purchase+receive → layer, two-receipt FIFO ordering, purchased sale → fifoUnitCost snapshot + NO SupplierOrder + sale movement, checkout rollback (stale price 409 → stock+layers restored exactly), supplierPrice → 999999 with COGS unchanged, layer-based inventory report valuation (1,200,000 not supplierPrice×stock), multi-layer sale weighted fifoUnitCost + full consumption, insufficient-layers fail-safe (no order, nothing invented), **concurrent checkouts 5+5 on [5×100k,5×200k] → each consumes its own layer, no oversell**, variant FIFO + isolation, pre-cutover supplierPrice fallback + **COGS reconciliation 2,180,000** (FIFO + fallback in one sum), refund → exact layer restoration + return_restock + idempotent (second refund 400, no double restore), customer cancel → exact layer restoration + cancellation_restock.
- **`tests/e2e/fifo-sale.spec.ts`** (Journey 20, desktop) — admin receives 10×50,000 through the real purchases API; the customer buys through the real storefront (add to cart → checkout → cash order) → order item `fifoUnitCost` 50,000, stock 10→9, layers 9×50,000, **no SupplierOrder**, sale movement −1/50,000; the admin refunds through the real `/admin/orders/[id]` modal → stock back to 10, layers restored at 50,000, `return_restock` movement; a consignment sale is verified to keep the SupplierOrder payout (amountOwed 120,000, no sale movement); the inventory report values the purchased product from its layers (10 × 50,000 = 500,000).
- **`scripts/run-regression.js`** — `verify-fifo` added after `verify-purchases` → **45 suites**; `regression.yml` renamed to **45-suite**.
- **Verified:** tsc 0 · `npm run check` exit 0 (4 pre-existing warnings) · Vitest **537/537** · `verify-fifo` **15/15** (hermetic ×3) · `verify-accounting` **14/14** + `verify-purchases` **16/16** (Phases A/B still green) · targeted FIFO E2E **4/4** (hermetic ×2) · Playwright **109/109** (chromium 97 incl. Journey 20 + mobile 12, `SMS_MOCK`+`ZARINPAL_MOCK` server) · full regression **45/45 PASS** (45 suites, `SMS_MOCK=1` only — the established convention).

## Session 82 Phase B (August 2026) — Procurement: PurchaseOrders, Receiving, FIFO Cost Layers & Movements

### The approved scope (Session 82 Phase B — the procurement domain on top of Phase A's accounting foundation; Phase C FIFO checkout consumption is explicitly NOT started)
- **New `PurchaseOrder` model** (additive; zero changes to Product/Order/SupplierOrder/AccountingConfig): `number` (P-YYYYMMDD-NNN), `supplier` (ref — the existing Supplier model), `purchaseDate`, `reference`, `notes`, `status` (`draft → ordered → partially_received → received`, plus `cancelled`), financial fields (`subtotal`, `discount`, `additionalCosts`, `total` = subtotal − discount + additionalCosts, `paymentStatus` unpaid|partial|paid, `amountPaid`), `items[]` (product ref, `variantId`, name, variantLabel, `quantity`, `receivedQuantity`, `unitCost`, `lineTotal`), `receipts[]` (append-only history: key, itemId, quantity, receivedAt, receivedBy), `createdBy`, `cancelledAt/cancelledBy/cancellationReason`. **Sourcing invariant:** only `sourcing: "purchased"` products may be purchased — a purchase attempt for a consignment product → clear validation error, no stock/layer mutation (Phase A invariant preserved). **Model change ⇒ dev-server restart required** (done).
- **Critical accounting rules enforced:** creating a PurchaseOrder does NOT create inventory; **paying does NOT create inventory**; **only receiving creates stock + FIFO cost layers** — every received quantity creates an exact `InventoryCostLayer` (unit cost from the PurchaseItem, NEVER the current `supplierPrice` — never silently substituted) + an append-only `InventoryMovement` of type `receipt`. Product/variant `stock` remains the fast-access cache, incremented through the existing atomic `stockVersion` optimistic-locking pattern.
- **Partial + idempotent receiving (`POST /api/admin/purchases/[id]/receive`):** per-item quantities must be ≥1 and ≤ outstanding; over-receipt → 409 with no partial state; client-supplied per-receipt `key` makes retries **idempotent** (repeated same-key request returns the already-applied receipt, no double stock/layers — verified: receiving the same quantity twice cannot duplicate inventory); concurrent duplicate-key claims are detected and treated as an idempotent retry (E11000-style guard on `receipts.key`); since MongoDB is standalone (no multi-doc transactions), the route mutates PurchaseOrder + product layers/stock with the failure window analyzed and the idempotency key as the reconciliation mechanism.
- **Purchase lifecycle:** `draft → ordered` (PATCH action: `order`), receiving transitions `partially_received`/`received` derived purely from received-vs-ordered totals (`src/lib/purchase-math.ts` — 15 hermetic unit tests: totals, outstanding, deriveStatus, derivePaymentStatus, receiveDeltaOk, buildPurchaseNumber), `cancel` allowed from draft/ordered (cancellation never destroys already-received inventory/layers/movements — historical receipt data is preserved). **Payment is financially separate from receiving** — `POST /api/admin/purchases/[id]/pay` records `amountPaid`/`paymentStatus` only; it never touches stock or layers. No purchase returns in v1 (reserved for a later phase).
- **Admin-only API (all `requireRoleOrError(["admin"])` + `PURCHASE_WRITE_LIMIT` 60/actor/15min):** `GET/POST /api/admin/purchases` (list with status/payment filters + pagination; create with server-side validation + totals recomputed from items), `GET/PATCH /api/admin/purchases/[id]` (detail incl. receipts + remaining; PATCH: order/update-when-draft/cancel), `POST .../receive`, `POST .../pay`. Malformed ObjectId → 400; validation BEFORE the rate limiter; rate-limit keys self-cleaned by the verify suite + regression runner sweep (`purchase-write`).
- **Purchases report (Session 81 subsystem extended):** new `getPurchasesReport` aggregation service (`src/lib/reports.ts`) + `REPORT_SLUGS.purchases` + `GET /api/admin/reports/purchases` + Excel export sheet — rows: purchase number, supplier, purchase date, status, payment status, ordered/received/outstanding quantity, subtotal, discount, additional costs, total, amount paid, amount outstanding. `parseReportFilters` gained an additive `kind: "purchase"` option (purchase status/payment label sets); report UI (`report-config`, `report-filters`, `summary-cards`, `/admin/reports/[report]` page) branches on the purchases meta. No fake tax/shipping/procurement fields.
- **Admin UI:** sidebar «خریدها» → `/admin/purchases` (list with status/payment filters + pagination), `/admin/purchases/new` (draft creation: supplier picker, product picker — **`sourcing` filter added to the admin products GET + `useAdminProducts` so only purchased-sourcing products are selectable**, line items with live line-total math), `/admin/purchases/[id]` (detail: line table with Ordered/Received/Remaining, receive form with remaining-aware max + receive-all, pay form, order/cancel actions; client-side invalid-quantity prevention — the API remains authoritative). Reuses existing Card/Button/Badge/Input/Textarea/PaginationControls conventions; zero new UI library.
- **Bugs found & fixed during implementation:** (1) the new-purchase product picker read `productsData.products` but `useAdminProducts` returns a `PaginatedResponse` whose array is under `.data` → the picker would have rendered empty (fixed); (2) impure `Date.now()` in render for the receive-key (moved into handlers — SSR-hydration-safe); (3) E2E ZWNJ mismatch on «پیش‌نویس» labels → ZWNJ-tolerant regex helper in the journey; (4) E2E beforeAll 429/400 on the accounting-init wizard → same `clearInitKeys` self-healing convention `verify-accounting.js` uses + leftover `accountingconfigs` singleton cleared (idempotent fixtures); (5) E2E layer assertion expected one layer of 10 but partial receiving creates two layers (4 + 6) — corrected to the exact two-layer shape.

### Tests & verification
- **`tests/unit/purchase-math.test.ts`** — **15 hermetic tests** (line totals, subtotal, total with discount/additional-costs, outstanding/received/ordered sums, status derivation incl. over-receive guard, payment-status derivation, receive-delta validation, purchase-number format). Vitest **528/528** (513 + 15).
- **`scripts/verify-purchases.js`** — **16/16 real-API** (self-cleaning, PREFIX'd, own `purchase-write` key sweep): unauthenticated → 401, non-admin → 403, create purchase (draft), read, update, order → `ordered`, **partial receipt (stock + exact FIFO layer + receipt movement)**, **full receipt (received status, remaining 0)**, **over-receipt → 409 (no partial state)**, stock increase via the atomic stockVersion pattern, exact cost-layer creation (unit cost from the PurchaseItem, never current supplierPrice), receipt movement creation, **payment does not change inventory** (no layers/movements/stock delta), **receipt does not change sourcing** (consignment stays consignment), **consignment purchase rejection** (400, no stock/layer mutation), cancellation preserves received inventory, **idempotent repeated receipt** (same key → no double stock/layers). Also covers purchases report: filtered rows + Excel export sheet. Run twice hermetic.
- **`tests/e2e/admin-purchases.spec.ts`** (Journey, desktop) — admin creates a draft purchase → views it → receives a partial quantity → verifies Remaining updates → receives the rest → verifies `received` status + product stock in the admin products list + purchases report visibility. Idempotent fixtures (409-safe create, ZWNJ-tolerant Persian matchers, init-key self-clearing, `accountingconfigs` singleton cleanup on beforeAll). Teardown extended (`tests/e2e/helpers/db.ts`): `purchaseorders` + `inventorymovements` cleanup + `purchase-write`/`accounting-init`/`accounting-config` rate-limit keys + the `accountingconfigs` singleton.
- **`scripts/run-regression.js`** — `verify-purchases` added after `verify-accounting` → **44 suites**; `regression.yml` renamed to **44-suite** (the verify suite self-cleans its own `purchase-write` keys; the runner's pre-suite sweep already covers the shared login/OTP/conversation/accounting keys).
- **Verified:** tsc 0 · `npm run check` exit 0 (4 pre-existing warnings) · Vitest **528/528** · `verify-purchases` **16/16** (hermetic ×2) · `verify-accounting` **14/14** (Phase A regression, still green with the new model) · `verify-payment-retry` **13/13** + `verify-variants-e2e` **32/32** (re-verified on the real-sandbox server — see note below) · targeted E2E **4/4** (hermetic ×2) · Playwright **93/93 chromium + 12/12 mobile** (mock server) · full regression **44/44 PASS** (44 suites, real-sandbox server with `SMS_MOCK=1` only).
- **Environment note:** the full regression must run against a server started with `SMS_MOCK=1` only — `ZARINPAL_MOCK=1` breaks `verify-payment-retry` (asserts real-sandbox `StartPay` URL + `S...` authority) and `verify-variants-e2e` (bogus-authority failure semantics). This matches the long-standing Session 59/60 convention. Two earlier regression attempts failed 18 suites for exactly this reason (server started with `ZARINPAL_MOCK=1`); after restarting without it: `verify-payment-retry` 13/13, `verify-variants-e2e` 32/32, full regression **44/44**.
