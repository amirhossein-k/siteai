...
> **Docs gap:** milestones from Sessions 69–87 are not all listed here (Sessions 85–87 were recorded for the first time in Session 88, reconstructed from their commit messages) — see CHANGELOG.md for the authoritative chronological record.
...
- [x] **Excel export fixed (Session 88)** — the generic summary sheet is skipped for `profitability` (it caused `Cannot read properties of undefined (reading 'orders')` → HTTP 500); the branch is now self-contained and emits **خلاصه سودآوری · آبشار سودآوری · سودآوری محصولات · تحلیل هزینه‌ها · روند** with numeric KPI/waterfall cells; the foundation's product-sheet contract and the conditional category sheet are preserved
- [x] **Own report metadata (Session 88)** — no longer reuses `PNL_META`: «سودآوری» title + profitability-specific description + `filters: { preset: true, trendGroup: true }`; **the P&L branch is untouched**
- [x] **Trend Group UI (Session 88)** — روزانه/هفتگی/ماهانه segmented control, rendered only when `meta.filters.trendGroup`, writing `?group=day|week|month`; a11y (`role="group"` + `aria-label` + `aria-pressed`)
- [x] **Trend bucket correctness (Session 88)** — every bucket now carries its OWN `from`/`to` (was: the overall report window); contiguous + non-overlapping; **zero-fill** of empty periods; **expense-only** periods kept; labels match Mongo `$dateToString` (`%Y-%m-%d` / `%G-W%V` using a real ISO week-year, incl. the `2027-01-01 → 2026-W53` transition / `%Y-%m`); 3000-bucket defensive cap; pure helpers `computeTrendBuckets()` / `assembleTrendBuckets()` extracted for unit testing
- [x] **Accessibility (Session 88)** — `scope="col"` + `aria-sort` headers wrapping real buttons, `sr-only` sort-direction text, `aria-hidden` decorative icons, textual alternatives for the waterfall/trend/KPI/health surfaces; no new dependency
- [x] **Accounting safety** — COGS expression untouched (`fifoUnitCost` → historical `supplierPrice` snapshot → 0); current `Product.supplierPrice` never used for historical profitability; **no `OrderItem` category snapshot, no migration/backfill**; P&L/FIFO/inventory/checkout/payment untouched
- [x] **Tests** — unit trend bucket boundaries (day/week/month) + zero-fill + expense-only + `group` parsing; **new offline XLSX test** (builds the real workbook and re-parses it → exactly the five sheets, numeric cells — regression-guards the 500); `verify-reports.js` extended (profitability JSON shape, `netSales − cogs = grossProfit`, `grossProfit − operatingExpenses = netProfit`, trend groups/boundaries/Σ trend = KPI, invalid `group` → 400, parsed export workbook)
- [x] **Verified** — tsc 0 · `npm run check` exit 0 · Vitest **623/623** (+20) · targeted 68/68 · `git diff --check` clean
- [ ] **BLOCKED (must re-run)** — `verify-reports.js` over HTTP, Playwright, and the full 49-suite regression: **MongoDB was unreachable for the whole session** (`MongooseServerSelectionError`; TCP connects, handshake refused host-side)
- [ ] **Deferred** — **historical category profitability**, which requires a `category` snapshot on the embedded `OrderItemSchema` plus a backfill decision (migration-class, explicitly out of scope). `categories` stays `[]` with an honest empty state.

### Customer Communication / Order Support (Session 68)
- [x] **`CustomerConversation` model** (additive; zero changes to User/Order/SupplierOrder/Supplier/Product) — customer/order/supplierOrder/supplier refs, optional product, category (general|delivery|product|refund|other), subject 1–120, status `open|pending|resolved|closed`, `customerUnread`+`staffUnread`, `lastMessageAt`/`lastMessagePreview`/`lastMessageFrom`, `messages[]` (sender ref + role + sanitized text + createdAt), `resolvedAt`/`closedAt`; **unique partial index `{supplierOrder} where active`** (one conversation per customer × supplier-order — checkout already splits per supplier, so cross-supplier leakage is structurally impossible); queue index `{supplier,status,lastMessageAt:-1}`; **model change ⇒ restart required** (done)
- [x] **Status machine (pure, unit-tested)** — `open→pending` (staff) · `pending→open` (customer) · `→resolved` (customer/admin) · `→closed` (customer closes own / admin) · reopen (customer/admin); message on `closed` → 400; message on `resolved` auto-reopens
- [x] **11 additive endpoints** (customer `/api/conversations*`, admin `/api/admin/conversations*`, supplier `/api/supplier/conversations*`) — all `requireRoleOrError` + server-side ownership + **same-404 not-found/not-owned**; order ownership via `Order.findOne({_id, customer: token.id})` + paid-only eligibility; sender identity always from the session; supplier derived from the SupplierOrder; malformed ObjectId → 400; validation-before-rate-limit (create 5/user/15min, message 15/actor/15min)
- [x] **Notifications via the existing `notifyOrderEvent` facade** — new `support` category + `support_message` type; dedupe key `conversation_<id>_<messageId>` (exactly one per recipient); **templated message, no content in payloads**; customer messages notify the conversation's supplier.user only; staff messages notify the customer; SSE bell + inbox `support` tab automatic
- [x] **UI** — customer `/support` + `/support/[id]` (account-menu «ارتباط با مشتری», profile card, order-detail button) · admin `/admin/support` (sidebar «پشتیبانی», filters + search, reply, resolve/close/reopen) · supplier `/supplier/support` (sidebar «ارتباط با مشتری», own threads only, reply-only) · shared `src/components/support/` components + `ui/textarea`
- [x] **Real-time v1** — SSE bell + 15s detail polling + focus refetch; **no WebSockets**
- [x] **Out of scope (v1)** — text-only, no external chat integration, no AI, no anonymous chat, supplier reply-only
- [x] `scripts/verify-customer-support.js` **23/23 real-API** wired into `run-regression.js` (**39 suites**); Journey 17 E2E; 28 unit tests
- [x] **Verified** — tsc 0 · check exit 0 · Vitest **254/254** · `verify-customer-support` **23/23** · Playwright **52/52** (chromium 40 incl. **Journey 17** + mobile 12) · full regression **39/39 PASS**
